# SNN ECG 4-Class Classification Accelerator IP Core 최종 보고서
...
아날로그 검증은 MATLAB AFE+ADC 사전설계, XMODEL-aligned LTspice 실제 schematic 구현·검증, SystemVerilog XMODEL 행동모델·통합 검증의 세 단계로 구성된다. 원본 개발은 teammate repositories에서 관리하지만, 최종 보고서 작성에 필요한 회로·코드·표·그림은 이 repo에도 evidence mirror로 보존한다. 동일한 10초 ECG 10,000-sample LTspice-XMODEL 비교에서 98.74%가 ±5 LSB, 99.89%가 ±10 LSB 이내였고 MAE 0.6445 LSB, zero-lag correlation 0.999518, clipping 0을 얻었다. 디지털 accelerator의 구현 경계는 이 flow가 제공하는 signed 12-bit ECG stream부터 시작한다. 본 결과는 실제 전극 기반 ECG 측정, physical AFE PCB 측정, ADC silicon 측정, CMOS layout/post-layout 검증 또는 clinical diagnosis validation을 의미하지 않는다.
...
본 보고서는 upstream analog validation과 **signed 12-bit ECG stream 이후의 digital accelerator IP**를 하나의 end-to-end 설계 흐름으로 연결하되, 각 검증의 역할을 분리한다. 목표는 높은 계산량의 generic neural network가 아니라, 검증된 1 kSPS ECG stream을 받아 60초 snapshot과 30분 final decision을 처리하는 low-resource biomedical streaming accelerator IP를 구현하고 검증하는 것이다.
...
전체 merged-paper flow는 MATLAB nominal pre-design, LTspice schematic verification, AFE+ADC XMODEL implementation, signed 12-bit ECG stream, SNN Snapshot Readout, Final Membrane Readout, RTL/IP/FPGA validation으로 이어진다. 이 repo는 전체 보고서 evidence를 연결하되 구현 소유권은 아래 표처럼 분리한다.
...
| LTspice XMODEL-aligned schematic verification | ±1.65 V 실제 R/C/op-amp 회로, AC/transient/S&H/ADC/stress 검증 | `.asc`, `.net`, op-amp library, metric tables, execution manifest | MATLAB 사양을 schematic으로 구현하고 XMODEL이 재현할 회로 계약을 고정 |
...
## 4. Analog AFE+ADC Verification and Digital Input Contract

### 4.1 단계별 검증 논리

아날로그 chain은 `HPF 0.482 Hz -> IA x201 -> 60 Hz active Twin-T notch -> LPF 150 Hz -> 12-bit ADC`로 구성한다. MATLAB, LTspice와 XMODEL은 같은 회로를 다루지만 검증 질문이 다르다.

| 단계 | 검증 질문 | 완료 결과 |
|---|---|---|
| MATLAB 사전설계 | 목표 cutoff, gain, notch, ADC 범위가 적절한가? | nominal frequency response와 ADC reference 고정 |
| LTspice 구현·검증 | R/C, op-amp, ±1.65 V, S/H를 포함한 schematic에서도 사양이 유지되는가? | AC, 10초 ECG, ADC mapping, S/H와 35개 nominal/stress run 완료 |
| SystemVerilog XMODEL | 검증된 schematic 동작을 mixed-signal/RTL simulation에서 재현하는가? | XMODEL 회로망과 1 kSPS ADC 구현, 10,000-code LTspice 상관 완료 |

![Figure A-1. Analog AFE architecture](reports/final/figures/analog_validation/01_analog_afe_architecture.svg)

### 4.2 MATLAB 사전설계에서 LTspice 실제 회로로의 이관

MATLAB은 HPF, IA, notch, LPF와 ADC의 nominal design reference를 제공한다. 이 사양을 XMODEL-aligned LTspice graphical schematic으로 옮겨 U1~U6 ±1.65 V 전원, 전용 op-amp abstraction, `ECG+=patient100`, `ECG-=0 V`, 50 µs input update, 1.000 ms first aperture와 1 kSPS sampling을 구현했다.

![Figure A-2. MATLAB-LTspice full AFE frequency-response comparison](reports/final/figures/analog_validation/03_matlab_ltspice_afe_response.png)

![Figure A-4. XMODEL-aligned LTspice schematic](reports/final/figures/analog_validation/02_ltspice_xmodel_aligned_schematic.jpg)

| 항목 | 목표 | LTspice 측정 |
|---|---:|---:|
| HPF -3 dB | 0.4823 Hz | 0.481174 Hz |
| IA gain at 10 Hz | 201 V/V | 200.594 V/V |
| 60 Hz notch | 60 Hz rejection | -83.557 dB |
| Notch minimum | 60 Hz | 59.9995 Hz, -95.435 dB |
| LPF -3 dB | 150.15 Hz | 150.211 Hz |
| Settled AFE output | ±1.65 V 이내 | -0.0540~+0.2466 V |
| ADC clipping | 0 | 0 / 10,000 |

LTspice 26.0.1에서 nominal, AC/fine-notch, ADC mapping, timestep convergence, DC/baseline, 50/60 Hz PLI, R/C mismatch, GBW와 VOS를 포함한 35개 run을 실행했으며 fatal/warning signature는 0건이었다. 고정 MATLAB reference와 LTspice의 1초 이후 ADC 비교는 MAE 0.678 LSB, RMS 2.225 LSB, correlation 0.998591이었다. MATLAB digital reference와 LTspice analog Twin-T는 bit-exact 대상이 아니라 design-intent consistency를 확인하는 단계다.

### 4.3 LTspice에서 SystemVerilog XMODEL로의 이관

LTspice로 고정한 topology와 parameter는 `xreal`, `resistor`, `capacitor`, `vcvs`, `vlimit` primitive를 사용하는 [`ecg_afe_xmodel.sv`](validation/afe_ltspice_xmodel_aligned/reference/xmodel_fixed_4756a50_subset/analog/ecg_afe_xmodel.sv)로 작성했다. [`ecg_afe_xmodel_op.sv`](validation/afe_ltspice_xmodel_aligned/reference/xmodel_fixed_4756a50_subset/analog/ecg_afe_xmodel_op.sv)는 GBW/VOS를, [`ecg_afe_xmodel_mm.sv`](validation/afe_ltspice_xmodel_aligned/reference/xmodel_fixed_4756a50_subset/analog/ecg_afe_xmodel_mm.sv)는 R/C mismatch를 주입한다. ADC는 `negedge clk_samp`에서 ±1.65 V 입력을 0~4095로 양자화하고 signed 12-bit stream으로 전달한다.

동일한 10초 `patient100` 입력과 1 kSPS sampling으로 XMODEL과 LTspice S/H ADC code 10,000개를 비교했다.

![Figure A-8. XMODEL-LTspice ADC quantitative comparison](reports/final/figures/analog_validation/10_xmodel_ltspice_adc_metrics.png)

![Figure A-9. ADC-code agreement by error range](reports/final/figures/analog_validation/09_xmodel_ltspice_adc_agreement.png)

| 항목 | 결과 |
|---|---:|
| Compared samples | 10,000 |
| Exact match | 56.49% |
| Within ±1 LSB | 91.19% |
| Within ±5 LSB | 98.74% |
| Within ±10 LSB | 99.89% |
| Mean / mean absolute error | +0.0221 / 0.6445 LSB |
| RMS / maximum absolute error | 1.3020 / 13 LSB |
| Zero-lag correlation | 0.999518 |
| Best lag / clipping | 0 sample / 0 |

12-bit ±1.65 V에서 5 LSB와 10 LSB는 ADC 입력 기준 약 4.03 mV와 8.06 mV다. 본 보고서는 ±5 LSB를 nominal agreement band, ±10 LSB를 extended QRS-edge band로 사용한다. 10,000개 중 11개만 ±10 LSB를 초과했고 최대 오차는 13 LSB였으며, 잔여 오차는 급격한 QRS edge의 input-grid/sub-sample solver timing 부근에 집중되었다. 따라서 XMODEL은 bit-exact 모델로 주장하지 않지만 LTspice의 nominal AFE+ADC 동작을 높은 정합도로 재현한 mixed-signal/RTL integration model로 사용한다.

전체 파형, QRS zoom, per-sample error, histogram, 60 Hz dense sweep와 figure provenance는 [`reports/final/analog_validation_result.md`](reports/final/analog_validation_result.md)에 정리했다.

### 4.4 Digital input contract

XMODEL 이후의 digital input contract는 **signed 12-bit ECG stream**이다.

| 항목 | 값 |
|---|---:|
| Input sample format | signed 12-bit stream |
| Sample rate | 1 kSPS |
| Snapshot window | 60,000 samples per 60-second snapshot |
...
| 검증 개념 | 의미 | 소유 위치 |
|---|---|---|
| Analog model correlation | MATLAB design, LTspice schematic와 XMODEL 행동모델의 단계별 정합 | upstream teammate repos, mirrored here |
| Digital full-top XSim expected outputs | locked full-top RTL이 만든 board-facing `final_pred` / `final_mem` expected output | this digital repo |
| Vitis/MicroBlaze board replay vs expected outputs | MicroBlaze + AXI sample feeder가 30분 stream을 replay하고 board output을 XSim expected와 비교 | this digital repo |
| AFE-to-locked RTL integration evidence | AFE+ADC XMODEL output stream을 locked RTL에 태워 digital expected와 비교 | XMODEL teammate repo, using canonical cadence |
...
아날로그 검증은 MATLAB 사전설계, LTspice schematic verification, SystemVerilog XMODEL implementation의 단계별 evidence로 정리했다. 이 repo는 그 보고서용 사본과 상관 결과를 보존하고, signed 12-bit ECG stream contract와 `sample_gap_cycles=2` canonical full-top XSim cadence를 기준으로 locked RTL/IP/FPGA 검증을 수행한다.

Hardware validation은 Python golden, full-top XSim expected output, Vivado implementation, IP-XACT packaging, Vitis/MicroBlaze board replay로 이어진다. 특히 board replay는 최종 locked RTL/IP/bitstream 기준에서 final_test 36개 30분 stream을 FPGA board로 replay하고, UART transcript와 comparison CSV를 통해 digital expected output과 final_pred/final_mem 36/36 일치를 보였다는 점에서 digital integration evidence로 해석한다.


## 11. Limitations

- Source ECG record는 이미 digitized public dataset이다.
- `vin` reconstruction과 analog 원본 개발은 upstream teammate repos에서 관리되며, 이 repo는 MATLAB/LTspice/XMODEL 보고서 evidence mirror와 signed 12-bit stream 이후의 digital validation을 보존한다.
- 본 repo는 raw analog ECG acquisition, physical AFE PCB validation, ADC silicon validation, CMOS layout/post-layout validation, clinical diagnosis validation을 주장하지 않는다.
- MATLAB/LTspice/XMODEL 결과는 model-based verification이며 physical acquisition 또는 silicon measurement로 해석하지 않는다.
- Validation 32/32 = 100.00%는 model-selection 성능이며 최종 generalization 성능으로 주장하지 않는다.
- Board replay는 strict final_test 36개 30분 case 전체에 대해 수행했지만, physical analog validation은 아니며 digital RTL/IP replay equivalence evidence이다.
- Board replay final_pred/final_mem exact match는 모두 36/36으로 보고한다.


## 12. Conclusion

본 repo는 MATLAB/LTspice/XMODEL AFE+ADC evidence와 signed 12-bit ECG-stream SNN accelerator IP의 **digital hardware validation evidence**를 연결한다. Analog 원본 개발은 upstream teammate repos에서 유지하고, 이 repo는 보고서용 회로·표·그림·코드 사본과 locked strict record-wise protocol, RTL/XSim, Vivado/IP-XACT, Vitis/MicroBlaze board replay evidence를 제공한다.

최종 모델은 train/validation으로 lock하고 final_test를 1회만 평가했으며, Python/XSim/Vivado/IP-XACT/Vitis evidence가 동일 locked model 기준으로 연결되어 있다. 대회 제출과 merged paper 관점에서는 MATLAB 사전설계 -> LTspice 실제 구현·검증 -> SystemVerilog XMODEL의 analog evidence가 signed 12-bit stream provenance를 제공하고, 그 stream 이후는 accelerator IP와 board-level digital replay evidence로 이어진다.
...
- Analog validation: `reports/final/analog_validation_result.md`
