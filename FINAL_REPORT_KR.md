...
본 프로젝트는 AFE+ADC를 거친 ECG stream을 저전력 RTL 회로에서 처리하여 NSR / CHF / ARR / AFF 4개 class를 분류하는 **SNN 기반 장시간 ECG 4-Class Classification Accelerator IP Core**를 구현하고 검증하는 것을 목표로 한다.
...
-> 60초 Snapshot Readout이 60초 class/evidence spike 생성
-> 30분 Final Membrane Readout이 class neuron membrane에 흥분성/억제성 자극 누적
...
| Locked final layer XSim | final_test 36 cases, final_pred/final_mem mismatch 0 |
| Locked Vitis/MicroBlaze board flow | NSR/CHF/ARR/AFF 각 1건 30분 replay 완료, final_pred/final_mem 4/4 match |
...
```text
공개 ECG digitized record
-> code / 200000 기반 analog-equivalent vin reconstruction
...
-> signed 12-bit .mem
-> SNN ECG Accelerator IP Core
-> Python / XSim / Vivado / Vitis board replay
-> NSR / CHF / ARR / AFF output
```
...
- [Performance Baseline](docs/PERFORMANCE_BASELINE_KR.md): Python baseline, RTL cycle counter, Vivado resource/power estimate를 정리한다.
...
- [Full-Record Board Replay with Vitis/MicroBlaze](docs/FULL_RECORD_BOARD_REPLAY_VITIS_KR.md): Vitis MicroBlaze + UART chunk-ACK flow로 1,800,000-sample `.mem` full record 1건을 실제 board에서 replay하고 Python/XSim expected와 exact match한 transcript를 정리한다.
...
60초 Snapshot Readout 검증 데이터셋:

```text
60s_afe_datasets/afe_output_xmodelmatch_curated_v2_128_64_64_balanced
```

30분 장시간 ECG 4-Class Accelerator IP Core 검증 데이터셋:
...
위 표는 `seed=20260808` strict record-wise split의 30분 chunk 수 기준이다. class별 chunk 수는 train / validation / test = 17 / 8 / 9로 균형화했고, `source_record_id`와 label-stripped `physical_record_id` 기준으로 train / validation / test 간 overlap 0을 확인한다. 동일 source record에서 생성된 여러 30분 chunk가 존재할 수 있지만, 같은 source record에서 나온 chunk가 서로 다른 split으로 섞이지 않도록 구성한다.
...
4. NSR/CHF/AFF에서는 같은 원천 record에서 여러 30분 chunk가 생성될 수 있으므로 split boundary는 chunk가 아니라 `source_record_id` 기준으로 관리한다.
5. Final Membrane parameter는 train / validation split에서만 확정하고, test split은 parameter lock 이후 최종 1회 평가 기준으로 사용한다.

## 3. 전체 RTL 모델 구조

장시간 ECG 4-Class Accelerator IP Core는 두 계층으로 구성된다.

```text
Layer 1: 60초 Snapshot Readout
...
Layer 2: 30분 Final Membrane Readout
...
-> 60초 Snapshot Readout segment_done
-> timer_mem reset
-> 다음 60초 snapshot 시작
```
...
## 5. 60초 Snapshot Readout 개요

60초 Snapshot Readout은 60초 AFE+ADC ECG stream을 받아 NSR / CHF / ARR / AFF 중 하나를 출력하는 SNN-inspired RTL classifier이다.
...
60초 Snapshot Readout은 feature vector classifier가 아니다. 각 feature spike/count가 class neuron membrane에 fixed signed weight로 흥분성 또는 억제성 자극을 준다.
...
최종 구조 정리는 기존 C24 folded spike readout을 유지하면서 **EERG direct class-membrane contribution을 제거**한 것이다. EERG direct path 제거는 validation에서 불필요한 자극 경로를 줄이고 test 성능을 유지했기 때문에 최종 구조에 반영했다.

## 6. Snapshot Feature Block 상세 설명

이 절은 대회 보고서에서 가장 중요하다. 장시간 ECG 4-Class Accelerator IP Core의 60초 Snapshot Readout은 ECG를 먼저 거대한 software feature vector로 만든 뒤 외부 classifier에 넣는 방식이 아니다. ECG stream에서 작은 event neuron들이 차례로 발화하고, 그 발화가 rhythm 또는 morphology evidence가 되어 class neuron membrane에 흥분성/억제성 자극을 넣는 구조이다.
...
### 6.9 EERG Gate: 후보 탐색용 gate, 최종 direct readout에서는 제거
...
하지만 60초 Snapshot Readout에서는 이 EERG direct class-membrane contribution을 제거했다. 이유는 다음과 같다.

1. EERG가 일부 validation case에서는 도움이 되었지만, 최종 구조에서는 direct 자극 경로를 제거해도 test 성능이 유지되었다.
2. 직접 ARR boost 경로가 남아 있으면 feature 설명이 불필요하게 복잡해지고, 특정 edge case에 과도하게 의존하는 구조가 된다.
3. 최종 구조는 더 단순한 “feature spike -> class membrane” 경로를 유지하기 위해 EERG direct boost를 끈 모델로 확정했다.

따라서 보고서에서 EERG는 다음처럼 표현하는 것이 정확하다.

```text
EERG는 후보 탐색 중 검토한 episodic ARR rescue gate였지만,
60초 Snapshot Readout의 최종 class membrane 직접 자극 경로에서는 제거했다.
```
...
## 7. Snapshot 후보군 C01-C32와 최종 Readout 선택 과정
...
이후 최종 구조에서는 C24 계열 folded readout을 기반으로 유지하되, EERG direct class-membrane 자극을 제거한 후보를 선택했다.

## 8. 60초 Snapshot Readout XSim 성능
...
4. 따라서 30분 Final Membrane Readout에서 반복되는 ARR evidence를 누적해 보완한다.

## 9. 30분 Final Membrane Readout

30분 Final Membrane Readout은 60초 Snapshot Readout 뒤에 붙는 30분 chunk-level readout이다. 60초 Snapshot Readout의 feature threshold, class weight, gate/boost는 고정하고, 30분 Final Membrane Readout만 별도 탐색/검증했다.

### 9.1 직관적 구조

60초 Snapshot Readout은 60초마다 class spike를 낸다.
...
30분 Final Membrane Readout은 30분 동안 30개의 snapshot class spike를 class별 final neuron membrane에 누적한다.
...
따라서 30분 Final Membrane Readout은 보조 evidence neuron membrane도 누적한다.
...
30분 Final Membrane Readout은 완전한 생물학적 SNN이라고 주장하지 않는다. 정확한 표현은 다음이다.
...
5. Locked model 기준 MicroBlaze full-record replay bitstream/XSA/ELF를 새로 생성했고, NSR/CHF/ARR/AFF 각 1건의 30분 UART replay transcript와 expected-vs-board CSV를 생성했다. 네 case 모두 full-top XSim과 final_pred 및 final_mem이 bit-exact로 일치했다.
...
1. Snapshot은 `60초 Snapshot Readout`로 고정
2. EERG direct class-membrane contribution 제거
3. 30분 final layer는 `30분 Final Membrane Readout`로 고정
...
위 `search_*` 이름의 Python 파일은 중간 탐색 산출물이 아니라, 최종 Python 등가모델과 XSim expected 결과 생성을 위해 보존한 고정 모듈이다.
...
최종 보고서에는 최종 채택된 검증 기준과 산출물만 남긴다. RTL/Python/XSim functional benchmark와 strict record-wise 최종 성능을 구분하고, strict record-wise 결과는 아래 dataset과 Final Membrane 확정 절차를 따른 결과만 사용한다.

### 14.1 Strict record-wise dataset
...
AXI/IP-XACT packaging 산출물과 feeder IP 산출물은 `ip_repo/` 아래 `component.xml` 및 `xgui`로 확인된다. Locked model 기준 MicroBlaze full-record replay system은 bit/XSA/ELF까지 다시 생성했고 timing을 만족했다. 실제 board replay는 NSR/CHF/ARR/AFF 각 1건을 수행했으며, final_pred와 final_mem이 4/4 full-top XSim과 bit-exact로 일치했다.

### 14.4 AFE+ADC XMODEL 산출물

공개 ECG dataset은 이미 digitized record이므로 실제 raw analog ECG를 복원한 것이 아니다. 최종 flow는 signed code를 `vin_v = code / 200000`으로 해석해 PWL-equivalent 입력을 만들고, nominal HPF 0.482 Hz, instrumentation gain x201, 60 Hz notch, LPF 150 Hz, 12-bit ADC quantization을 통과시킨 signed 12-bit `.mem` stream을 RTL/IP 입력으로 사용한다.
## 15. 한계와 향후 개선
...
2. class별 원천 record 수를 더 확보해 strict split의 test support를 늘리고, 특히 AFF 1-record test 한계를 줄이기
...
장시간 ECG 4-Class Accelerator IP Core strict record-wise record-majority accuracy: 16/19 = 84.21%
장시간 ECG 4-Class Accelerator IP Core locked XSim mismatch: pred 0, mem 0
장시간 ECG 4-Class Accelerator IP Core board replay: final_pred/final_mem 4/4 exact match
Vivado DSP usage: 0
Vivado BRAM usage: 0
```
