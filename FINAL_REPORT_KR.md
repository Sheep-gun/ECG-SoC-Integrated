...
AXI/IP-XACT packaging 산출물과 feeder IP 산출물은 `ip_repo/` 아래 `component.xml` 및 `xgui`로 확인된다. MicroBlaze smoke system은 bit/XSA, XSDB MMIO transcript, Vitis-built ELF, UART PASS transcript까지 확인했다. 추가로 Vitis MicroBlaze + UART chunk-ACK full-record replay flow를 구성했고, test NSR case 0의 1,800,000 samples를 실제 board에서 replay하여 `SNN_ECG_FULL_REPLAY_BOARD_PASS`와 Python/XSim expected-vs-board exact match를 확인했다. 다만 전체 test split board replay가 끝난 것은 아니므로, full board replay 완료 범위는 1-case integration evidence로 제한한다.

Full-record board replay 결과:
...
| board_final_pred | 0 (NSR) |
| expected_final_pred | 0 (NSR) |
| board_final_mem | NSR/CHF/ARR/AFF = 31 / 0 / 1 / 0 |
| expected_final_mem | NSR/CHF/ARR/AFF = 31 / 0 / 1 / 0 |
| PASS marker | `SNN_ECG_FULL_REPLAY_BOARD_PASS` |
| transcript path | `reports/board_replay/transcripts/test_case0_nsr_uart_full_replay.txt` |
| comparison path | `reports/board_replay/comparisons/test_case0_nsr_expected_vs_board.csv` |
...
4. MicroBlaze smoke/IP evidence, UART PASS transcript, test NSR case 0 full-record board replay PASS transcript는 있으나, 전체 split에 대한 board replay batch는 아직 TODO이다.
...
6. 30분 Final Membrane Readout test ARR recall은 6/9로 남은 병목이다.
