...
AXI/IP-XACT packaging 산출물과 feeder IP 산출물은 `ip_repo/` 아래 `component.xml` 및 `xgui`로 확인된다. MicroBlaze smoke system은 bit/XSA, XSDB MMIO transcript, Vitis-built ELF, UART PASS transcript까지 확인했다. 다만 full 30분 board replay transcript는 아직 없으므로 smoke/system build evidence와 full replay 완료 주장을 분리한다.
...
4. MicroBlaze smoke/IP evidence와 UART PASS transcript는 있으나 full 30분 board replay는 아직 TODO이다.
