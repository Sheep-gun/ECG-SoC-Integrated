...
def parse_nominal():
    ...
    for name,vals,label in [("ltspice_xmodel_aligned_direct_adc_signed.mem",direct_signed,"XMODEL-aligned LTspice direct aperture"),("ltspice_track_hold_adc_signed.mem",th_signed,"LTspice Track-and-Hold")]:
    ...
    # This remains an LTspice candidate until the fixed-XMODEL correlation gate passes.
...
def parse_stress():
    ...
    for case in cases:
        ...
        reference_note = "reference=MM0 PLI60" if case.startswith("mm_") and case != "mm_0_pli60" else "reference=aligned nominal"
        for name,val,unit in vals: rows.append(metric(f"STRESS_{case}","XMODEL_ALIGNED",src,"TRAN",name,val,unit,evidence=ev,notes=reference_note))
