...
[확인된 사실] XMODEL-aligned LTspice direct stream과 LTspice S/H stream의 zero-lag MAE는 0.0037 LSB, RMS는 0.0755 LSB, maximum은 5 LSB, correlation은 0.9999984이고 best lag는 0 sample이다. 이는 내부 진단이며 XMODEL equivalence 판정이 아니다.
...
[후속 완료] Fixed XMODEL source를 licensed environment에서 실행한 team handoff 결과를 추가했다. 동일 10초 입력 10,000-sample의 LTspice S/H↔XMODEL 비교는 mean error +0.0221 LSB, MAE 0.6445 LSB, RMS 1.3020 LSB, maximum 13 LSB, correlation 0.999518, lag 0, clipping 0이다. 98.74%가 ±5 LSB, 99.89%가 ±10 LSB 이내이며 최신 figure와 통합 해석은 `../../../docs/MIXED_SIGNAL_VERIFICATION_KR.md`를 따른다.
