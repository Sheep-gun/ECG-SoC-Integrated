...
6. S/H clock은 `PULSE(0 5 900u 1u 1u 98u 1m)`이다. Switch가 확실히 off인
   1.000 ms 직후를 S/H valid phase로 사용한다. XMODEL-aligned LTspice direct ADC는
...
- 이 audit 작성 시점의 LTspice workspace에는 Questa/XMODEL executable이 없어 direct gate가 pending이었다. 후속 licensed team run에서 동일 10초 10,000-sample 상관을 완료했고 MAE 0.6445 LSB, correlation 0.999518, ±5 LSB 98.74%, ±10 LSB 99.89%를 얻었다. 최신 통합 판정은 `../../../reports/final/analog_validation_result.md`를 따른다.
