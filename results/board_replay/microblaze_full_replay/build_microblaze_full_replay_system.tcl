...
create_project -force SNN_ECG_MB_FULL_REPLAY $proj_dir -part xc7a100tcsg324-1
...
set_property -dict [list CONFIG.C_BAUDRATE {230400} CONFIG.C_DATA_BITS {8} CONFIG.C_USE_PARITY {0}] [get_bd_cells axi_uartlite_0]
...
validate_bd_design
save_bd_design
write_bd_tcl -force "$out_dir/snn_ecg_mb_full_replay_bd.tcl"
set addr_fh [open "$out_dir/reports/system_bd_address.rpt" w]
puts $addr_fh "Long-window ECG 4-Class Accelerator MicroBlaze full-record replay address map"
...
make_wrapper -files [get_files "$proj_dir/SNN_ECG_MB_FULL_REPLAY.srcs/sources_1/bd/$bd_name/$bd_name.bd"] -top
add_files -norecurse "$proj_dir/SNN_ECG_MB_FULL_REPLAY.gen/sources_1/bd/$bd_name/hdl/${bd_name}_wrapper.v"
...
set bit_file "$proj_dir/SNN_ECG_MB_FULL_REPLAY.runs/impl_1/snn_ecg_mb_full_replay_wrapper.bit"
if {![file exists $bit_file]} {
    error "Bitstream not found: $bit_file"
}
file copy -force $bit_file "$out_dir/snn_ecg_mb_full_replay.bit"

if {![catch {write_hw_platform -fixed -include_bit -force -file "$out_dir/snn_ecg_mb_full_replay.xsa"} msg]} {
    puts "XSA=$out_dir/snn_ecg_mb_full_replay.xsa"
} else {
