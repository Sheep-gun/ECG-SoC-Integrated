...
Wearable·portable ECG는 일상에서 single-lead waveform과 rhythm 정보를 제공할 수 있다. 대표적인 소비자용 단일유도 ECG 앱의 규제 문서 사례에서는 sinus rhythm과 atrial fibrillation 중심의 rhythm-screening 범위를 제시한다. 이는 특정 제품 문서의 사례이며 모든 wearable이 동일한 기능을 가진다는 뜻이 아니다. 본 연구 문제는 공개 digitized ECG의 장시간 stream에서 local morphology와 long-term persistence를 함께 다루는 transparent hardware architecture이며, 제품별 clinical 성능과 본 project의 public-dataset accuracy를 직접 비교하지 않는다.
...
PhysioNet raw waveform은 public Git에 번들하지 않는다. 고정 버전·records·SHA256·license/citation과 외부 fetch 절차는 [`dataset_manifest.yaml`](datasets/dataset_manifest.yaml), [`DATASET_LICENSES.md`](datasets/DATASET_LICENSES.md), [`datasets/README.md`](datasets/README.md)에 있다. Locked `.mem`, split/evaluation CSV와 integration evidence는 retained derived artifacts다.
...
![Detailed digital architecture](figures/final/FIG-12_detailed_digital_architecture.svg)

Direct RTL audit에 따르면 pure accelerator는 1,800,000-sample raw input window를 저장하지 않고 고정 크기 event/state와 Final Membrane을 갱신한다 [CLM-023]. `avoided full raw-input window storage`는 21,600,000 bit=2,700,000 byte≈2.7 MB decimal이다. 상세 signal inventory와 한계는 [`STREAMING_STATE_MEMORY_KR.md`](docs/STREAMING_STATE_MEMORY_KR.md)에 있다.
...
```text
components/             three curated commit-pinned upstream snapshots
...
figures/final/          12 generated non-benchmark SVG figures including FIG-12
tables/                 classification, implementation, integration and streaming-state tables
benchmarks/             pending external benchmark placeholder only
reports/                technical manuscript, publication audits and integrity reports
...
세 component는 working tree가 아니라 fixed Git objects에서 curated export됐다. 모든 retained imported file은 upstream fixed commit과 byte-identical하고 `artifact_manifest.csv`에 origin/path/size/SHA256이 기록된다. 생략한 upstream path는 두 exclusion registry에 남으며 fixed commits가 complete source snapshot의 authority다. 다음 명령은 metric과 figure를 다시 만들고 repository claim/integrity를 검사한다.
...
Application forms, Office lock file과 fixed-version PhysioNet raw dataset path는 정책상 제외됐고 hash가 [`excluded_upstream_paths.csv`](integration_evidence/excluded_upstream_paths.csv) 및 [`excluded_large_dataset_paths.csv`](integration_evidence/excluded_large_dataset_paths.csv)에 남아 있다. 최종 private HWP/PDF와 개인정보는 이 Git 밖에 둔다.
