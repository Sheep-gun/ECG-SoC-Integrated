...
*그림 2. 공개 ECG를 원본 기록 단위로 학습·검증·잠금 시험 데이터로 먼저 분리한다. 학습·검증 데이터로 MATLAB·XMODEL Front End를 검토한 뒤 디지털 모델과 RTL을 개발하고, 디지털 검증 기준을 충족할 때까지 RTL 설계를 수정한다. 기준을 충족하면 설계를 잠그고 격리해 둔 시험 데이터로 고정 분류기를 최초 1회 평가한다. 이후 RTL·IP·FPGA 구현과 XMODEL–RTL 통합 단계는 잠금 설계와의 등가성만 확인하며 모델·문턱값·구조 재조정을 허용하지 않는다. 아날로그 계층은 모델 기반이다. [근거: CLM-007, CLM-012, CLM-013, CLM-028; `source_of_truth/upstream_commits.yaml`; `components/digital_accelerator/configs/final_submission_locked_model.json`; `components/afe_xmodel/docs/integration_latest/afe_locked_rtl_integration_36case_compare.csv`]*
...
*그림 7. ECG+와 ECG−가 각각 HPF를 통과한 뒤 3-op-amp IA에서 합류하고, Active Twin-T 60 Hz notch, buffer를 포함한 150 Hz LPF, 12비트 ADC와 부호 있는 12비트 스트림으로 이어지는 AFE·ADC 신호 흐름. 입력 교란은 두 ECG 입력 노드에, R/C 불일치는 IA와 Twin-T에만 연결한다. 유한 GBW는 능동 연산증폭기 단계 전체에서 평가했고 VOS 스트레스는 IA 입력쌍에 적용했다. ADC 비이상성은 ADC 출력 코드 경계 한 곳에 주입한다. 실선은 신호 경로이고 점선은 주입 교란 또는 비이상 모델이다. 원본 LTspice schematic이 아니다. 실제 PCB·silicon·post-layout 결과를 뜻하지 않는다. [직접 근거: `components/matlab_prevalidation/matlab_afe_validation/docs/afe_adc_parameter_reference.md`; `components/afe_xmodel/analog/ecg_afe_xmodel.sv`; `source_of_truth/unresolved_artifacts.csv`]*
