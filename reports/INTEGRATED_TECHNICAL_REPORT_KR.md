...
장시간 ECG 감시에서는 병리적 특징이 기록 전체에 계속 나타나지 않고 특정 시간대, 일부 박동 또는 제한된 구간에서만 간헐적으로 나타날 수 있다. 따라서 개별 심박이나 수십 초 기록을 분류하는 문제와, 수시간에서 하루 이상 이어지는 Holter 기록에서 의미 있는 이상 구간을 찾아 전체 기록의 상태를 판정하는 문제는 구분해야 한다. 본 연구는 후자의 문제를 지향한다. 현재 구현의 60초 Snapshot 30개와 30분 입력은 공개 데이터 길이에 맞춘 검증 조건이며, 연구의 본질은 고정된 두 숫자가 아니라 장시간 입력을 순차 구간으로 압축하고 간헐적 질환 증거의 강도·출현 빈도·반복성과 장시간 일관성을 기록 단위 판정으로 연결하는 데 있다 [CLM-035, CLM-042].

### 2.1.1 개별 심박을 분류하는 SNN 연구

Amirshahi와 Hashemi는 개별 ECG 심박을 SNN으로 분류하였다[3]. 입력은 R-peak 전 0.25초와 후 0.45초로 잘라낸 한 심박이며, 한 심박을 여러 겹치는 구간으로 나눈다. 각 표본의 진폭은 진폭에 비례하는 발화율의 푸아송 스파이크 열(Poisson spike train)로 변환되고, 양·음 진폭은 서로 다른 입력 세포가 담당한다. STDP 계층은 스파이크가 발생한 시점에 따라 심박 형태 특징을 비지도 학습하고, R-STDP 출력 계층은 정답이면 보상, 오답이면 벌점을 주어 최종 심박 클래스를 학습한다. 이 연구는 STDP/R-STDP 기반 저전력 ECG 분류 가능성을 보여주지만 출력 단위가 **개별 심박**이며, 여러 시간 구간의 증거를 기록 단위 질환 클래스로 집계하지 않는다 [CLM-036].

### 2.1.2 연속 ECG의 사건 구동형 뉴로모픽 탐지

Bauer, Muir와 Indiveri는 연속 다채널 ECG를 비동기 이진 사건열로 바꾸고, DYNAP 뉴로모픽 처리기의 recurrent SNN reservoir와 사건 구동형 판독부로 처리하였다[4]. 각 병리 유형의 판독부는 뉴런 발화도의 가중합을 저역통과시키고, 어느 판독값이 문턱값을 넘으면 병리 유형의 존재를 알리는 이진 검출 신호를 낸다. 이는 긴 정상 구간 속 드문 이상을 상시 감시 방식으로 검출하는 중요한 선행 사례지만, 최종 출력은 이상 검출 신호이며 Snapshot별 다중 질환 증거를 기록 단위 클래스 상태로 누적하는 구조는 아니다 [CLM-037].

Chen, Tian, Yang과 Sawan은 레벨 교차 ADC(level-crossing ADC, LC-ADC)와 스파이킹 합성곱 신경망(SCNN)을 함께 설계하였다[5]. LC-ADC는 입력이 양자화 준위를 넘을 때만 방향을 포함한 스파이크를 만들고, SCNN은 이를 직접 받아 스파이킹 합성곱·완전연결 계층과 출력 스파이크 횟수로 클래스를 정한다. 공식 ISCAS 논문은 MIT-BIH에서 선택한 개별 심박의 N·SVEB·VEB·F 분류와 입력 표본 수 감소를 평가한다. 따라서 핵심은 감지부와 처리부 사이의 사건 표현과 입력 압축이며, 장시간 기록의 여러 국소 결과를 상위 상태로 집계한 연구는 아니다 [CLM-038].

### 2.1.3 긴 ECG에서 일부 위험 구간을 집계하는 연구

Shanmugam, Blalock과 Guttag은 약 48시간 ECG를 여러 구간의 집합으로 보고 환자 단위 심혈관 사망 위험을 예측하는 다중 인스턴스 학습(multiple instance learning, MIL)을 제안하였다[6]. R-peak 주위 1초 심박을 정렬한 뒤 여러 연속 심박을 하나의 인스턴스로 만들고, 인스턴스별 위험 확률 가운데 높은 20%의 평균을 환자 단위 점수로 사용한다. 이 집계는 고위험 환자라도 대부분의 심박은 정상처럼 보이고 일부 병리적 연속 심박의 비율이 더 높을 것이라는 가정을 명시한다. 긴 기록의 모든 부분을 동일하게 평균하지 않고 일부 강한 구간의 출현을 전체 결과로 연결한다는 점에서 본 연구와 개념적으로 가장 가깝다. 다만 결과는 소프트웨어 MIL 기반 이진 예후 위험도이고, 리듬·파형·질환별 증거를 고정 폭 RTL 상태로 누적하는 네 클래스 분류기는 아니다 [CLM-039].

### 2.1.4 가변 길이와 24시간 Holter의 상위 시간 통합

Zihlmann, Perekrestenko와 Tschannen은 9–61초의 가변 길이 단일유도 ECG를 정상 리듬, 심방세동 리듬, 기타 리듬, 잡음 기록의 네 클래스로 분류하였다[7]. 두 구조 모두 CNN으로 국소 특징을 만들며, 하나는 시간축 평균으로, 다른 하나는 3층 양방향 LSTM의 마지막 출력으로 기록 전체 특징을 통합한다. 이 연구는 CNN 기반 국소 특징과 평균/LSTM 기반 기록 단위 네 클래스 분류기가 이미 존재함을 보여준다. 따라서 본 연구의 차별성을 단순히 “4개 클래스”로 주장하지 않는다. 클래스 정의, 입력 길이, 집계 상태와 RTL 구현 방식이 다르다 [CLM-040].

이전에 DeepHHF로 언급된 연구는 Zvuloni 등 저자의 “Modeling day-long ECG signals to predict heart failure risk with explainable AI”로 확인하였다[8]. 이 논문은 2026년 npj Digital Medicine에 정식 출판됐으며 DOI는 10.1038/s41746-026-02835-8이다. 24시간 단일유도 Holter를 30초 구간으로 표상하는 인코더를 먼저 학습하고, 고정 인코더가 만든 시간 순서 특징을 Transformer 순차 통합부로 결합하여 전체 Holter의 5년 HF 위험 점수를 출력한다. 이는 짧은 구간 인코더와 상위 시간 통합부가 24시간 기록의 발작성 사건과 장기 변화를 통합할 수 있음을 보여주는 검증 가능한 동등 연구다. 그러나 이진 예후 소프트웨어이며 NSR·CHF·ARR·AFF 상태를 설명 가능한 RTL 누산값으로 만드는 구조는 아니다 [CLM-041].

### 2.1.5 선행연구 비교와 본 연구의 위치

| 연구 | 최종 판정 단위 | 국소 ECG 처리 | 장시간 집계 방식 | 구현 형태 | 본 연구와의 차이 |
|---|---|---|---|---|---|
| Amirshahi–Hashemi STDP/R-STDP[3] | 개별 심박 클래스 | R-peak 주위 심박, 진폭 스파이크 변환, STDP 특징 | 없음 | SNN 알고리즘 | 기록 단위 질환 판정 아님 |
| Bauer et al.[4] | 병리 유형 이진 검출 | 비동기 사건 변환, recurrent SNN | 판독부 발화도의 저역통과 | DYNAP 뉴로모픽 칩 | 다중 질환 기록 상태 누적 없음 |
| Chen et al.[5] | 개별 심박 부정맥 클래스 | LC-ADC 사건과 SCNN | 한 심박 안의 출력 스파이크 횟수 | 혼합신호/SNN 모의실험 | 입력 압축과 심박 분류가 중심 |
| Shanmugam et al.[6] | 환자 단위 심혈관 사망 위험 | 연속 심박 인스턴스 | 고위험 인스턴스 20% 평균 | 소프트웨어 MIL | 개념적으로 유사하나 이진 예후·비RTL |
| Zihlmann et al.[7] | 짧은 기록 단위 네 클래스 | 스펙트로그램 CNN 특징 | 시간 평균 또는 양방향 LSTM | 소프트웨어 심층학습 | 네 클래스는 존재하나 클래스·길이·구현이 다름 |
| DeepHHF[8] | 24시간 Holter의 5년 HF 위험 | 30초 구간 인코더 | Transformer 순차 통합부 | 소프트웨어 심층학습 | 장시간 통합은 유사하나 이진 예후·내부 판단 불투명 |
| 본 연구 | NSR·CHF·ARR·AFF 기록 단위 | 설명 가능한 리듬·파형 형태·질환 사건 | Snapshot 증거의 강도·빈도·반복·지속성을 Final 상태에 누적 | RTL/IP/FPGA | 고정 폭 상태와 mixed-signal-to-RTL 추적성 |

*표 2. 대표 관련 연구의 판정 단위와 장시간 집계 방식 비교. 과업·데이터셋·입력 길이·출력 단위가 서로 달라 정확도 순위 비교에는 사용할 수 없다. [근거: EXT-009~EXT-014; CLM-036~CLM-042; `docs/RELATED_WORK_HOLTER_ECG_KR.md`]*

기존 ECG SNN 연구는 주로 개별 심박을 분류하고, 사건 구동형 뉴로모픽 연구는 연속 ECG에서 병리적 사건 또는 이상 유무를 검출한다. 장시간 ECG 연구는 여러 심박 또는 짧은 구간의 특징을 심층학습이나 MIL로 통합하여 환자·기록 단위 위험도를 생성한다. 본 연구는 장시간 Holter ECG에서 간헐적으로 나타나는 질환성 Snapshot을 포착하고, 각 Snapshot의 리듬·파형 형태·질환별 증거의 강도와 반복성을 설명 가능한 고정 폭 RTL 상태에 누적하여 전체 기록의 NSR·CHF·ARR·AFF 클래스를 판정한다 [CLM-042].

검토한 대표 선행연구 범위에서는 NSR·CHF·ARR·AFF 기록 단위 분류, Snapshot별 질환 증거의 명시적 상태화, 장시간 증거 누적, RTL/IP/FPGA 구현과 MATLAB–XMODEL–RTL 추적성을 함께 적용한 사례를 확인하지 못하였다. 이는 문헌 전체에 동일 연구가 없다는 최초성 단정이 아니라, 위 여섯 편의 대표 원 논문과 공식 출판 경로를 기준으로 한 제한된 비교다 [CLM-042].
...
디지털 분류기 앞단의 목적은 공개 ECG 전압을 증폭만 하는 것이 아니라, 기준선 이동과 전원선 간섭을 줄이고 1 kSPS signed 12-bit 스트림으로 일관되게 넘기는 것이다. 현재 고정 component에는 README가 언급하는 LTspice `.asc`와 원본 회로 캡처가 실제로 포함되어 있지 않다. 따라서 그림 7은 원본 schematic이 아니라 MATLAB 파라미터 문서와 SystemVerilog XMODEL 정본을 바탕으로 다시 그린 설명용 재구성도다. 누락된 원본은 `source_of_truth/unresolved_artifacts.csv`에 기록하며, 본문은 이 재구성도를 제작된 회로도 또는 실제 회로 증거로 사용하지 않는다 [CLM-034].

![그림 7. AFE·ADC 회로 블록 재구성도](../figures/final/FIG-15_afe_adc_reconstructed_diagram.svg)

*그림 7. 고정 MATLAB 파라미터와 XMODEL RTL에서 재구성한 설명용 회로 블록도. 원본 LTspice schematic이 아니다. 실제 PCB·silicon·post-layout 결과를 뜻하지 않는다. [직접 근거: `components/matlab_prevalidation/matlab_afe_validation/docs/afe_adc_parameter_reference.md`; `components/afe_xmodel/analog/ecg_afe_xmodel.sv`; `source_of_truth/unresolved_artifacts.csv`]*
...
![그림 12. 디지털 아키텍처](../figures/final/FIG-12_detailed_digital_architecture.svg)

*그림 12. 독자 개념을 중심으로 정리한 디지털 아키텍처. 모듈 이름은 구현 확인용 보조 표기이며 실제 넷리스트를 대체하지 않는다.*
...
디지털 IP 구현 뒤에는 두 질문을 분리해 확인한다. 첫째, 동일한 30분 분류 작업에서 가속기가 제공하는 처리시간·처리량·전력·에너지 이득이 무엇인가. 둘째, AFE XMODEL이 만든 signed stream이 디지털 RTL과 FPGA 경계까지 변형 없이 전달되고 같은 내부 상태와 판정을 만드는가. 현재 첫 질문의 외부 benchmark 수치는 반입 대기 상태이며, 두 번째 질문은 SHA256과 bit-exact 비교로 검증하였다.

## 6.1 가속기 Benchmark 범위와 현재 상태
...
*표 12. 가속기 benchmark 대기 상태. 값이 비어 있다는 것은 0이라는 뜻이 아니다. [근거: CLM-018; benchmark placeholder]*
