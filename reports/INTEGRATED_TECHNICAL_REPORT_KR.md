...
Wearable 및 ambulatory ECG 시스템은 사용자가 일상 환경에서 단일유도 ECG를 기록하고 리듬 정보를 확인할 수 있게 한다.
...
대표 소비자 ECG의 AF/sinus screening과 본 연구의 NSR/CHF/ARR/AFF 분류는 class 수만 다른 동일 문제가 아니다.
...
Architecture는 complete raw window를 먼저 buffer하지 않고 sample마다 fixed-size event/state register를 갱신한다. 만약 30분 input 전체를 그대로 저장한다면 필요한 raw storage는 다음과 같다.
...
본 설계는 이 **avoided full raw-input window storage** 대신 previous sample, filter/membrane, RR token, feature counter와 class state를 유지한다.
...
전체 2.7 MB decimal raw input window를 먼저 저장하지 않고 current sample, previous state, beat/event counter와 class membrane을 갱신한다.
...
세 component를 fixed commit으로 import하고 1,890개 file에 SHA256 provenance를 기록하였다.
...
본 연구는 Holter형 장시간 ECG에서 local beat/morphology evidence와 long-term persistence를 함께 반영하기 위해 60초 Snapshot Readout과 30분 Final Membrane Readout을 결합한 multi-timescale SNN-inspired event/state architecture를 제안하였다. Public ECG를 1 kSPS signed 12-bit stream으로 통일하고, adaptive event, QRS LIF, RR prediction/variability, morphology, amplitude, ectopic-like와 delay evidence를 integer state로 변환하였다. 전체 1,800,000-sample raw window를 먼저 저장하지 않고 streaming state를 갱신하며, locked Final Membrane과 WTA가 네 public-dataset class를 결정한다.
...
[3] George Moody / PhysioNet, “MIT-BIH Normal Sinus Rhythm Database v1.0.0,” official dataset record, 1999-08-03. https://physionet.org/content/nsrdb/1.0.0/

[4] PhysioNet, “BIDMC Congestive Heart Failure Database v1.0.0,” official dataset record, 2000-10-14. https://physionet.org/content/chfdb/1.0.0/

[5] George Moody and Roger Mark / PhysioNet, “MIT-BIH Arrhythmia Database v1.0.0,” official dataset record, 2005-02-24. https://physionet.org/content/mitdb/1.0.0/

[6] George Moody and Roger Mark / PhysioNet, “MIT-BIH Atrial Fibrillation Database v1.0.0,” official dataset record, 2000-11-04. https://physionet.org/content/afdb/1.0.0/
