...
*그림 7. ECG 입력에서 HPF·IA·60 Hz notch·LPF·ADC·signed 변환을 거쳐 디지털 RTL로 이어지는 analog signal flow. 블록에는 단계명만 표시하며 R/C·이득·ADC 범위와 XMODEL 비이상성 조건은 본문에서 설명한다. 원본 LTspice schematic이 아니다. 실제 PCB·silicon·post-layout 결과를 뜻하지 않는다. [직접 근거: `components/matlab_prevalidation/matlab_afe_validation/docs/afe_adc_parameter_reference.md`; `components/afe_xmodel/analog/ecg_afe_xmodel.sv`; `source_of_truth/unresolved_artifacts.csv`]*
...
*그림 12. signed ECG에서 변화량, 강한 변화 사건, QRS LIF, 리듬·파형 증거, 60초 Snapshot과 Final Membrane을 거쳐 NSR·CHF·ARR·AFF를 판정하는 디지털 signal flow. 화살표는 왼쪽에서 오른쪽으로 진행하는 상위 단계만 나타내며, 세부 병렬 경로와 상태 갱신은 본문에서 설명한다. 실제 합성 netlist 배선을 뜻하지 않는다.*
