...
본 연구는 장시간 심전도(electrocardiogram, ECG)에서 짧은 구간의 박동·파형 정보와 장기 지속성을 함께 반영하는 다중 시간축 분류 구조를 제안하고, 이를 초당 1,000개의 signed 12-bit 표본값을 받는 streaming RTL accelerator IP로 구현·검증하였다. 회로는 현재 표본값과 직전 표본값의 차이에서 짧은 사건 신호(event)를 만들고, 반복되는 강한 사건을 막전위형 누적값에 더해 하나의 박동을 검출한다. 이어서 박동 사이 RR 간격의 규칙성과 변동성, 박동 주변의 기울기 전환·진폭·폭·에너지·말단 활동도를 작은 정수 상태로 압축한다. 이 정보는 60,000표본의 60초 Snapshot에서 국소 클래스 상태를 만들고, Snapshot 30개의 승자 횟수와 집계 특징은 30분 Final Membrane에 누적된다. 마지막에는 네 클래스 상태를 승자독식 방식으로 비교하여 NSR·CHF·ARR·AFF 가운데 하나를 출력한다. MATLAB nominal AFE+ADC, SystemVerilog XMODEL, signed-stream SHA256, Python 정수 기준 모델, RTL/XSim, Vivado, AXI/IP-XACT, Vitis/MicroBlaze와 FPGA 재생 검증을 하나의 근거 사슬로 연결하였다. 엄격한 원천 record 단위 분할에서 고정된 최종 시험 결과는 29/36=80.56%, record-majority는 16/19=84.21%였다. Pure RTL은 LUT 9,719, FF 5,038, BRAM 0, DSP 0으로 구현되었고 FPGA의 `final_pred`와 `final_mem`은 XSim 기준과 각각 36/36 일치하였다. 전체 1,800,000표본 원시 입력 구간을 저장하지 않는 구조이지만, 데이터베이스와 클래스의 결합, 실제 AFE/ADC·임상 검증 부재와 미완료 accelerator benchmark가 남는다. 따라서 본 결과는 임상 진단의 증명이 아니라 장시간 네 클래스 분류 과정을 설명 가능한 사건·상태 회로와 재현 가능한 RTL/IP/FPGA 산출물로 완결한 반도체 IP 공학 결과다.
...
이전 상태는 직전 클록의 `prev_sample`, 계산값은 `delta_calc/abs_delta_calc`, 상태 확정 시점은 수락된 표본값의 상승 에지다. `up_event`, `down_event`, `strong_event`, `slope_valid`는 기본적으로 0이며 조건을 만족한 표본에서만 한 클록 동안 1이 된다. 60초 구간이 시작되면 적응형 변화량 분포표와 12개 문턱 후보 계수기를 지운다. 고정된 최상위 설계는 적응형 후보 선택을 켜므로 초기 보정 표본에서 각 변화 크기 후보를 넘은 횟수를 센다. 이 가운데 목표 사건 밀도를 만족하는 문턱 후보를 선택한 뒤 `adaptive_ready`를 올린다. 선택 전에는 고정 사건 문턱값을 사용한다. 이렇게 만든 상승·하강·강한 사건은 박동 검출기와 DSCR, QRS MAF, RBBB-like 경로에 동시에 전달된다. 이 기능을 RTL에서는 `ecg_event_encoder_adaptive`가 담당한다.
...
**3) RR 간격 확정.** 첫 `beat_spike`가 들어오면 박동 사이 시간을 재는 토큰을 활성화하고 `token_age`를 0으로 만든다. 이후 수락된 표본값마다 `rhythm_tick`이 들어오면 나이를 포화 방식으로 1씩 증가시킨다. 다음 박동이 오면 현재 틱까지 포함한 `age_eval`을 `rr_interval`과 평가 레지스터에 복사하고 토큰 나이를 다시 0으로 만든다. 첫 박동에는 비교할 이전 박동이 없으므로 RR 출력도 만들지 않는다. 따라서 RR 간격은 운영체제 시간이 아니라 실제로 수락한 표본값의 개수다.
...
**5) RDM 연속 간격 변화량.** PNN이 반복 간격을 따르는지를 본다면 RDM은 바로 앞 두 RR 간격의 차이가 얼마나 큰지를 본다. 첫 유효 RR은 `prev_rr`만 초기화한다. 다음 RR부터 `abs(current_rr-prev_rr)`를 계산하고, 차이가 넘은 15개 문턱 수준을 온도계 코드처럼 낮은 단계부터 누적해서 표시한다. `rdm_level_spike[14:0]`는 넘은 문턱을 비트별로 나타내고 4비트 코드는 그중 가장 높은 수준을 나타낸다. 같은 클록에 `prev_rr`를 현재 RR로 바꾸므로 다음 비교에서는 이번 간격이 기준이 된다. 즉 PNN은 “다음 간격이 이전에 선택한 반복 중심을 따르는가”를, RDM은 “연속 간격끼리 얼마나 달라졌는가”를 Snapshot에 제공한다. 이 기능을 RTL에서는 `rdm_variability_neuron`이 담당한다.

**6) Early–late 쌍.** 첫 RR은 적응 기준 `rr_ref`를 초기화한다. 이후 RR이 기준보다 충분히 짧으면 early, 충분히 길면 late 패턴으로 표시하고, 직전 비정상 패턴을 `prev_pattern`에 보관한다. 현재와 이전 패턴이 모두 유효하고 서로 다르면 early→late 또는 late→early 조합이므로 `ectopic_pair_spike`를 발생시킨다. 매 간격마다 기준은 `|RR-ref|`를 오른쪽으로 이동한 작은 단계만큼 현재 RR 방향으로 움직인다. 정상 범위 RR은 쌍 상태를 바꾸지 않고, 60초 구간 시작에서 기준과 패턴을 지운다. 이 쌍 스파이크는 Snapshot 파형 형태·리듬 클래스 기여와 장시간 계수기에 쓰인다. 이 기능을 RTL에서는 `ectopic_pair_neuron`이 담당한다.
...
**1) DSCR 기울기와 방향 전환.** 파형이 한 방향으로만 움직이는지, 상승과 하강을 반복하는지를 알려면 매 순간의 유효 기울기 부호를 기억해야 한다. 원시 표본값끼리 바로 빼면 작은 잡음에도 부호가 쉽게 바뀔 수 있으므로, 회로는 고정소수점 필터 상태 `filt_mem`이 입력을 완만하게 따라가게 한다. 현재 코드를 소수부를 가진 형식으로 바꾸고 `filter_error=sample-filt_mem`, `filter_update=error>>>FILTER_SHIFT`, `filt_next=filt_mem+filter_update`를 계산한다. 갱신값의 부호가 상승·하강 방향이고 갱신 절댓값을 정수 기울기 입력으로 사용한다.
...
`clear=segment_start`에서 필터, 상승·하강 상태와 이전 유효 부호를 지운다. 60초 동안 `valid_slope_spike`와 `sign_flip_spike`는 각각 유효 기울기 횟수와 방향 전환 횟수에 누적되고, Snapshot의 파형 형태 클래스 상태에 부호 있는 기여값으로 전달된다. 이 기능을 RTL에서는 `dscr_spike_counter`가 담당한다.

**2) RAM 최대 진폭 코드.** 모든 표본을 대상으로 최고점을 찾으면 서로 다른 박동의 파형이 섞일 수 있으므로, 진폭 경로는 PNN이 예측한 다음 RR 중심 주변에서만 `ram_window_open`을 연다. 토큰이 활성이고 예측기가 유효하며 현재 토큰 나이가 예측 중심의 허용 범위 안일 때 관찰을 시작한다. 상승 에지에서 최대 코드, 박동 검출 표시와 후속 유지 상태를 초기화한다. 고정된 최상위 설계에서는 입력 정규화기가 꺼져 있고 RAM/QRS MAF 기준선 입력은 부호 있는 0이므로, 수락 표본마다 `max(adc_data-baseline,0)`의 양의 진폭을 사용한다.

여러 진폭 문턱 가운데 입력이 넘은 가장 높은 문턱의 번호에 1을 더해 작은 코드로 만든다. 각 표본값의 코드가 현재 최대 코드보다 크면 최대 레지스터를 교체한다. RTL 이름 `r_peak_abs`는 오해하기 쉽지만 현재 구현에서 이 레지스터는 전체 해상도 진폭이 아니라 0으로 확장한 **최대 진폭 코드**를 보관한다. 관찰 구간 안에서 박동이 발생하면 post-hold 계수기를 시작해 박동 뒤 일정 표본까지 계속 최대 코드를 찾는다. 유지 구간 종료 시 박동이 있었다면 `ram_amp_code`와 한 주기의 `ram_amp_spike`를 출력한다. 최상위 설계는 60초 동안 코드 합과 유효 코드 횟수를 누적하여 Snapshot의 진폭 증거로 사용하고, 코드 합은 30분 Final Membrane 집계에도 전달한다. 파형 전체를 보관하지 않고 한 박동의 최고점을 작은 정수 하나로 압축하는 구조다. 이 기능을 RTL에서는 `ram_peak_accumulator`가 담당한다.

**3) QRS MAF의 박동 전후 관찰.** 같은 RR 간격을 가진 박동이라도 QRS 주변에서 활동이 얼마나 오래 이어지는지, 방향이 몇 번 바뀌는지, 기준선에서 얼마나 크게 벗어나는지가 다를 수 있다. 이를 구하려면 검출된 박동 한 시점만 보는 것이 아니라 박동 전후의 제한된 표본 이력을 함께 보아야 한다. 회로는 다음과 같이 박동 전 120표본과 박동 후 100표본을 연결해 폭·복잡도·에너지·선행 활동을 계산한다.

- **박동 전 120표본:** `pre_strong_sr`, `pre_flip_sr`와 120개의 8비트 에너지 코드 이력을 한 표본씩 이동한다. 강한 사건 횟수, 기울기 방향 전환 횟수와 에너지 합을 미끄러지는 관찰 구간 방식으로 유지하고, 강한 사건의 시간 FIFO에서 가장 오래된 위치와 가장 최근 위치를 추적한다.
...
- **Pre-QRS 활동도:** 박동 직전에 강한 사건이 하나 이상 있었는지, 방향 전환이 반복되었는지, 박동 전 에너지 합이 충분했는지를 저장해 둔 이전 상태로 판단하고 `pre_qrs_bump_spike`를 만든다.

박동 후 관찰 구간 종료 클록에는 폭·복잡도·에너지를 평가 레지스터에 저장한다. 다음 파이프라인 단계에서 적응 기준과 비교 조건을 계산하고, 그 다음 클록에 `qrs_maf_valid_spike`, 폭·복잡도·에너지 이상 스파이크와 pre-QRS bump를 출력한다. 폭·에너지 기준도 이 출력 단계에서 이전 기준을 바탕으로 다음 기준을 확정한다. `segment_start`는 박동 전 이력, 관찰 구간 상태, 기준과 파이프라인을 모두 초기화한다. 출력 스파이크와 코드는 Snapshot의 파형 형태 점수와 60초 계수기에 들어가고, 관련 횟수와 코드 합은 Final Membrane의 장시간 집계로 전달된다. 이 순서를 통해 “이전 관찰→평가값 저장→조건 계산→출력/다음 기준 확정”이 분리된다. 이 기능을 RTL에서는 `qrs_maf_neuron`이 담당한다.
...
가장 늦게 표시된 가설이 폭 대리지표가 되고, 폭 조건과 말단 활동 횟수 조건을 각각 넓은 파형 사건과 말단 활동 사건으로 바꾼다. 두 조건이 함께 참이면 박동 단위 `rbbb_like_beat_spike`가 발생한다. 60초 동안 넓은 파형, 말단 활동, 두 조건의 결합 횟수를 누적한다. 60초 경계에서는 결합 박동이 반복되었는지, PNN 불일치가 지나치게 높지 않은지, 설정에 따라 높은 RDM 억제 조건을 확인한 뒤 구간 단위 증거를 낸다. 이 증거는 Snapshot 클래스 점수에 들어가며 반복 횟수는 Final Membrane의 장시간 집계에도 전달된다. 따라서 한 박동의 늦은 활동도가 곧 클래스 증거가 되지 않고 반복성과 리듬 맥락을 거친다. 이 기능을 RTL에서는 `rbbb_qrs_delay_bank`가 담당한다.
...
**60초 Snapshot 누적.** `class_score_neurons`는 PNN 일치/불일치, RDM 코드, DSCR 기울기/전환, RAM 코드, ectopic pair, QRS MAF 이상 사건, pre-QRS bump와 RBBB-like 구간 사건을 입력으로 받는다. 각 사건에는 네 클래스에 대한 고정된 부호 기여값이 있다. 양의 기여는 해당 클래스 상태를 올리는 흥분, 음의 기여는 내리는 억제다. 리듬과 파형 형태의 국소 상태는 분리되어 갱신되다가 판독에서 더해진다. 동시에 박동 수, 사건 수, 코드 합과 일정 비율·평균 조건을 60초 계수기로 유지한다.
...
30번째 Snapshot에서도 현재 60초 구간의 값을 포함한 `*_next`를 `dec_*` 레지스터에 먼저 저장한다. 다음 파이프라인 단계는 이렇게 고정한 30분 집계값만 사용한다. 기본 막전위의 시작값은 클래스별 Snapshot 승자 횟수다. 따라서 Final Membrane은 단순 투표에 그치지 않고, 승자 횟수만으로 설명되지 않는 리듬·파형 형태의 지속성을 구조적 증거와 함께 본다.
...
최상위 `snn_ecg_30min_final_top`은 `IDLE→CORE_RESET→SEG_START→RUN→SEG_DONE→FLUSH→COMMIT→DONE` 순서의 FSM으로 동작한다. RUN 상태에서만 `sample_valid&&sample_ready`가 참인 표본을 수락한다. 표본 처리 뒤에는 내부 사건 신호가 다음 레지스터까지 전달될 시간을 주고, 60,000번째 수락 표본에서 구간 종료 상태로 이동한다. 판독 파이프라인에 남은 값을 모두 반영한 뒤 COMMIT 상태에서 Final 계층에 Snapshot을 전달한다. 이 절차를 30번 반복하고, 30번째 확정에서만 `chunk_done`을 올려 최종 출력을 유효하게 만든다.
...
| 박동 사이 | 토큰 나이, RR 예측기, RDM 직전 RR, ectopic 기준 | 박동 또는 리듬 틱 | 60초 구간 시작 |
| 박동 관찰 구간 | RAM 최대 코드, QRS MAF 전/후 상태, 지연 나이 | 표본/박동/관찰 종료 | 박동/60초 구간 |
