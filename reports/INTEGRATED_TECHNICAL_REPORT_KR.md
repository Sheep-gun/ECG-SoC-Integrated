...
*그림 2. 공개 ECG를 원본 기록 단위로 학습·검증·잠금 시험 데이터로 먼저 분리한다. 학습·검증 데이터로 MATLAB·XMODEL Front End를 검토한 뒤 디지털 모델과 RTL을 개발하고, 디지털 검증 기준을 충족할 때까지 RTL 설계를 수정한다. 기준을 충족하면 설계를 잠그고, 격리해 둔 시험 데이터는 잠금 최종시험에서 최초 1회만 사용한다. 이후 RTL·IP·FPGA 구현 검증과 XMODEL–RTL end-to-end 통합 검증을 거쳐 최종 결과를 정리한다. 최종시험 결과는 설계에 되먹임하지 않으며, 아날로그 계층은 모델 기반이다. [근거: CLM-007, CLM-012, CLM-013, CLM-028; `source_of_truth/upstream_commits.yaml`; `components/digital_accelerator/configs/final_submission_locked_model.json`; `components/afe_xmodel/docs/integration_latest/afe_locked_rtl_integration_36case_compare.csv`]*

이 workflow에 적용되는 중요한 평가 원칙은 **설계·검증 반복과 최종 성능 평가의 분리**다. 원본 기록 단위 분할 직후 잠금 시험 데이터를 격리하고, MATLAB과 XMODEL·RTL 설계에는 학습·검증 데이터만 사용한다. MATLAB에서는 공칭 필터·이득·동적 범위와 기준 벡터를 확인하고, XMODEL에서는 간섭과 비이상성 및 부호 스트림 인계를 확인한다. 이후 디지털 모델과 RTL의 기능 일치를 점검하고, 기준을 만족하지 못하면 이 개발 단계에서 수정한다. 설계를 잠근 뒤에는 격리된 시험 데이터를 최초 1회만 평가하며 그 결과로 AFE 파라미터나 RTL 판정 구조를 다시 조정하지 않는다. 구현·보드 재생과 XMODEL–RTL end-to-end 검증은 잠금 설계의 추적성과 기능 등가성을 확인하는 단계다.
...
![그림 7. Analog AFE·ADC signal flow](../figures/final/FIG-15_afe_adc_signal_flow.svg)

*그림 7. ECG+와 ECG−가 각각 HPF를 통과한 뒤 3-op-amp IA에서 합류하고, Active Twin-T 60 Hz notch, buffer를 포함한 150 Hz LPF, 12-bit ADC와 signed 12-bit stream으로 이어지는 AFE·ADC signal flow. 상단의 `Input Disturbance Injection`, `R/C Mismatch Model`, `Op-Amp GBW / VOS Model`, `ADC Non-Ideality Injection`은 점선으로 해당 검증 지점에 연결한다. 실선은 신호 경로이고 점선은 주입 교란 또는 비이상 모델이다. 원본 LTspice schematic이 아니다. 실제 PCB·silicon·post-layout 결과를 뜻하지 않는다. [직접 근거: `components/matlab_prevalidation/matlab_afe_validation/docs/afe_adc_parameter_reference.md`; `components/afe_xmodel/analog/ecg_afe_xmodel.sv`; `source_of_truth/unresolved_artifacts.csv`]*
...
*그림 11. signed ECG는 `ΔECG Calculation`, `Strong-Event Detector`, `QRS LIF Neuron`을 차례로 거친다. 검출된 박동은 위쪽 `Rhythm Feature Path`와 아래쪽 `Morphology Feature Path`로 분기되며, 두 경로의 증거는 `Feature Accumulation & Class Scoring`에서 합류한다. `60 s Snapshot Membrane` 30개는 `30-Snapshot Accumulation`을 거쳐 `30 min Final Membrane`으로 누적되고 NSR·CHF·ARR·AFF 기록 클래스를 판정한다. 실제 합성 netlist 배선을 뜻하지 않는다.*
...
![그림 12. 기능 등가성](../figures/final/FIG-08_signed_stream_handoff.svg)

*그림 12. 입력 byte 동일성, 기준 RTL 상태 동일성과 정답 표지 일치의 분리. [직접 근거: `components/afe_xmodel/docs/integration_latest/afe36_sha256_bitidentity.csv`; `components/afe_xmodel/docs/integration_latest/afe_locked_rtl_integration_36case_compare.csv`; `components/digital_accelerator/reports/final/board_replay_36_batch_summary.json`]*
...
![그림 13. 분류 결과](../figures/final/FIG-10_classification_summary.svg)

*그림 13. 검증 결과는 모델 선택용으로 분리하고 고정 최종 시험 결과를 중심에 둔 분류 요약.*
...
![그림 14. Post-route FPGA 구현과 hierarchy 배치](../figures/publication/FIG-P05_vivado_implementation/device_view_annotated_publication.svg)

*그림 14. 실제 Vivado Device View에 고정 routed checkpoint의 계층별 배치 셀 좌표를 정합한 post-route FPGA 구현 Figure. (a)는 SNN accelerator, MicroBlaze, local memory, 표본 공급기, AXI·UART·interrupt logic을 색으로 구분한 전체 system이며, (b)는 SNN accelerator에 속한 배치 셀만 분리 표시한다. (c)는 pure RTL과 MicroBlaze 통합 system의 자원·timing 범위를 구분한다. 색상은 실제 배치 좌표를 나타내지만 pblock 경계를 뜻하지 않으며, 본 그림은 ASIC layout이 아니다.*

![그림 15. MicroBlaze IP Integrator Block Design](../figures/publication/FIG-P05_vivado_implementation/microblaze_block_design.svg)

*그림 15. MicroBlaze, AXI interconnect, 표본 공급기, SNN accelerator, UART와 IRQ 연결. Vivado `write_bd_layout`의 native SVG를 raster 변환 없이 회전·여백만 정규화한 벡터다.*

![그림 16. Worst setup timing path](../figures/publication/FIG-P05_vivado_implementation/worst_setup_path.svg)

*그림 16. Vivado `write_schematic`으로 직접 export한 post-route 최악 setup path. data path delay 9.810 ns, requirement 10.000 ns, slack 0.097 ns이며 MicroBlaze 통합 system 범위다.*
