...
*그림 2. 공개 ECG와 세 component의 provenance를 고정한 뒤 MATLAB 공칭 검증, XMODEL 비이상성 검증, streaming RTL, Vivado/IP/FPGA 통합을 순차적으로 확인하고 잠금 최종시험으로 넘어가는 전체 workflow. 각 판단 단계의 반복 화살표는 공칭·모델·구현 오류 수정만을 뜻한다. LOCK 아래의 final-test 결과는 위 설계 단계로 되먹임하지 않는다. 아날로그 계층은 모델 기반이며 물리 측정 결과가 아니다. [근거: CLM-007, CLM-012, CLM-013, CLM-028; `source_of_truth/upstream_commits.yaml`; `components/digital_accelerator/configs/final_submission_locked_model.json`; `components/afe_xmodel/docs/integration_latest/afe_locked_rtl_integration_36case_compare.csv`]*

그림 2에서 중요한 구분은 **설계·검증 반복과 최종 성능 평가의 분리**다. MATLAB에서는 공칭 필터·이득·동적 범위와 기준 벡터를 확인하고, XMODEL에서는 간섭과 비이상성 및 signed stream 인계를 확인한다. 이후 정수 reference와 RTL의 기능 일치, FPGA 통합의 `final_pred`·`final_mem` 일치를 차례로 점검한다. 이 과정에서 발견된 구현 오류는 해당 단계에서 수정할 수 있지만, 잠금 이후 한 번 수행한 final-test 결과로 AFE 파라미터나 RTL 판정 구조를 다시 조정하지 않는다.
...
디지털 분류기 앞단의 목적은 공개 ECG 전압을 증폭만 하는 것이 아니라, 기준선 이동과 전원선 간섭을 줄이고 1 kSPS signed 12-bit 스트림으로 일관되게 넘기는 것이다. 현재 고정 component에는 README가 언급하는 LTspice `.asc`와 원본 회로 캡처가 실제로 포함되어 있지 않다. 따라서 그림 7은 원본 schematic이 아니라 MATLAB 파라미터 문서와 SystemVerilog XMODEL 정본에서 확인한 신호 순서·설계값·비이상성 주입 위치를 왼쪽에서 오른쪽으로 정리한 설명용 signal flow다. 누락된 원본은 `source_of_truth/unresolved_artifacts.csv`에 기록하며, 본문은 이 그림을 제작된 회로도 또는 실제 회로 증거로 사용하지 않는다 [CLM-034].

![그림 7. Analog AFE·ADC signal flow](../figures/final/FIG-15_analog_signal_flow.svg)

*그림 7. ECG 차동 입력에서 HPF·IA·60 Hz notch·LPF·ADC·signed 변환을 거쳐 RTL stream으로 이어지는 analog signal flow. 위쪽은 XMODEL의 교란·아날로그·ADC 비이상성 주입 위치이고, 아래쪽은 MATLAB 공칭 기준과 XMODEL 검증 역할이다. 원본 LTspice schematic이 아니다. 실제 PCB·silicon·post-layout 결과를 뜻하지 않는다. [직접 근거: `components/matlab_prevalidation/matlab_afe_validation/docs/afe_adc_parameter_reference.md`; `components/afe_xmodel/analog/ecg_afe_xmodel.sv`; `source_of_truth/unresolved_artifacts.csv`]*
...
*그림 12. signed 12-bit ECG 표본에서 변화량·강한 사건·QRS 발화를 만든 뒤, 박동·리듬 경로와 파형 형태 병렬 경로를 60초 Snapshot에서 합치고 30개 Snapshot을 Final Membrane으로 누적하는 디지털 signal flow. 고정 RTL의 기능적 상태 이동을 설명하기 위한 그림이며 실제 합성 netlist 배선을 뜻하지 않는다.*
