...
본 시스템의 평가 단위는 30분 ECG다. 네 클래스는 길이가 같은 단일 cohort가 아니라 서로 다른 공개 데이터베이스에서 구성되며, ARR에 사용한 MIT-BIH Arrhythmia Database는 48개의 30분 excerpt를 제공한다[11]. 클래스마다 실제 관찰 길이를 동일하게 유지하기 위해 모든 입력을 `60초 Snapshot × 30개`로 구성하였다 [CLM-035].
...
DeepHHF[8]는 24시간 Holter를 30초 구간으로 나누고 Transformer로 합쳐 5년 심부전 위험을 예측한다. **구간 분할과 장시간 통합 흐름은 가장 유사**하지만, DeepHHF의 답은 미래 위험도이고 본 연구의 답은 입력 기록의 네 클래스다. 본 시스템의 평가 입력은 네 공개 데이터에 공통으로 적용할 수 있는 30분으로 구성하였다 [CLM-035, CLM-041, CLM-042].
...
**평가 길이.** 네 데이터베이스에 동일하게 적용할 수 있는 실제 기록 길이는 30분이다. 특정 클래스만 반복하거나 빈 값을 채우지 않고 같은 시간을 비교하도록 `60초 Snapshot × 30개`로 통일했다 [CLM-035].
...
분류 결과는 서로 다른 PhysioNet 원천에서 구성한 NSR·CHF·ARR·AFF 데이터에 적용된다. 원천 record 단위 분할로 동일 record의 직접 누출을 차단했으며, 최종 시험은 36개 구간과 19개 record로 구성된다. 클래스와 데이터베이스가 일대일로 대응하므로 이 수치는 해당 공개 데이터 구성에 대한 성능이다.
