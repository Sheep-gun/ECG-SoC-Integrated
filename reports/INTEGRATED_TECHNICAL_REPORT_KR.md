...
장시간 심전도(electrocardiogram, ECG) 분석에서는 한 박동의 모양뿐 아니라 박동 간격과 파형 특징이 수십 분 동안 어떻게 반복되는지도 함께 보아야 한다. 본 연구는 공개 ECG에서 NSR·CHF·ARR·AFF 네 클래스를 자동 분류하기 위해 장시간 입력을 제한 길이 Snapshot으로 순차 처리하고, 간헐적으로 강하게 나타나는 질환 증거의 강도·빈도·반복성과 장시간 일관성을 기록 단위 상태로 누적하는 구조를 제안하였다. 먼저 MATLAB에서 HPF, 3-op-amp IA, 60 Hz notch, LPF와 12-bit ADC의 공칭 응답·동적 범위·기준 벡터를 고정하였다. 이를 ±1.65 V LTspice schematic으로 구현해 AC, 10초 ECG, S/H, ADC mapping과 mismatch·GBW·VOS·PLI를 검증한 뒤, 같은 회로 계약을 SystemVerilog XMODEL로 작성하였다. 동일한 10초 ECG 10,000개 ADC code를 LTspice와 XMODEL에서 비교한 결과 MAE 0.6445 LSB, zero-lag correlation 0.999518, ±5 LSB 98.74%, ±10 LSB 99.89%, clipping 0을 얻었다. 이 스트림을 받는 디지털 IP는 인접 표본값의 차이에서 사건을 만들고, 반복되는 강한 변화를 막전위형 상태에 누적해 박동을 검출한다. RR 간격의 규칙성과 변동성, 기울기 방향 전환, 최대 진폭, QRS 주변의 폭·복잡도·에너지를 고정 폭 정수 상태로 압축한다. 현재 공개 데이터 검증에서는 60초 증거 30개를 Final Membrane에 누적해 최종 클래스를 선택한다. 구현은 Python 정수 기준 모델, RTL/XSim, Vivado, AXI/IP-XACT, Vitis/MicroBlaze와 FPGA 재생으로 이어지며, AFE 출력에서 보드 입력까지 SHA256과 내부 상태를 비교하였다. 원천 record 단위로 분리한 고정 최종 시험의 30분 구간 정확도는 29/36=80.56%, record-majority 정확도는 16/19=84.21%였다. Pure RTL은 LUT 9,719, FF 5,038, BRAM 0, DSP 0으로 구현되었고 FPGA의 `final_pred`와 `final_mem`은 XSim 기준과 각각 36/36 일치하였다. 저장된 30분 ECG 처리에서는 단일 스레드 Exact C++ 1,777.699800 ms와 cycle-derived FPGA core 54.012600 ms를 비교해 32.912687배의 처리시간 비율을 얻었다. 이는 물리 보드에서 측정한 speedup이 아니며 live 판정은 여전히 30분 관찰을 필요로 한다. 클래스와 데이터베이스의 결합, 실제 PCB/ADC·임상 검증 부재와 물리 보드 timing·전력 미측정이 남아 있으므로, 본 결과는 임상 진단 성능이 아니라 MATLAB→LTspice→AFE·ADC XMODEL→디지털 가속기 IP→FPGA로 이어지는 장시간 ECG 분류 시스템의 공학적 구현과 검증으로 한정한다.
...
검토한 대표 선행연구 범위에서는 **간헐적 질환 구간을 찾고 그 반복 증거를 모아 NSR·CHF·ARR·AFF 기록 클래스를 판정하는 목적**과 MATLAB–LTspice–XMODEL–RTL–FPGA 검증 흐름을 함께 적용한 사례를 확인하지 못하였다. 이는 문헌 전체에 동일 연구가 없다는 최초성 단정이 아니라, 위 여섯 편의 대표 원 논문과 공식 출판 경로를 기준으로 한 제한된 비교다 [CLM-042].
...
전체 신호 흐름은 `공개 ECG → MATLAB 공칭 AFE+ADC → LTspice schematic 검증 → SystemVerilog XMODEL → 1 kSPS signed 12-bit stream → 사건·상태형 디지털 코어 → RTL/IP/FPGA 재생`이다. MATLAB은 공칭 필터·이득·ADC 범위와 기준 벡터를 제공한다. LTspice는 실제 R/C·op-amp abstraction·전원·S/H·ADC mapping으로 회로가 같은 목표를 만족하는지 확인한다. XMODEL은 검증된 회로 계약을 RTL과 함께 실행하면서 전원선 간섭, 기준선 변동, 부품 오차, 연산증폭기와 ADC 비이상성 및 장시간 부호 스트림을 검토한다. 디지털 코어는 이 스트림을 표본값 단위로 받아 Snapshot과 Final Membrane을 계산한다 [CLM-049~CLM-052].

![그림 2. 전체 연구·검증 workflow](../figures/final/FIG-02_research_workflow.svg)

*그림 2. 공개 ECG를 원본 기록 단위로 학습·검증·잠금 시험 데이터로 먼저 분리한다. 학습·검증 데이터로 MATLAB→LTspice→XMODEL Front End를 검토한 뒤 디지털 모델과 RTL을 개발하고, 디지털 검증 기준을 충족할 때까지 RTL 설계를 수정한다. 기준을 충족하면 설계를 잠그고 격리해 둔 시험 데이터로 고정 분류기를 최초 1회 평가한다. 이후 RTL·IP·FPGA 구현과 XMODEL–RTL 통합 단계는 잠금 설계와의 등가성만 확인하며 모델·문턱값·구조 재조정을 허용하지 않는다. 아날로그 계층은 schematic/behavioral model 기반이다. [근거: CLM-007, CLM-012, CLM-013, CLM-028, CLM-049~CLM-052; `validation/afe_ltspice_xmodel_aligned/README.md`; `components/digital_accelerator/configs/final_submission_locked_model.json`; `components/afe_xmodel/docs/integration_latest/afe_locked_rtl_integration_36case_compare.csv`]*

이 workflow에 적용되는 중요한 평가 원칙은 **설계·검증 반복과 최종 성능 평가의 분리**다. 원본 기록 단위 분할 직후 잠금 시험 데이터를 격리하고, MATLAB·LTspice·XMODEL과 RTL 설계에는 학습·검증 데이터를 사용한다. MATLAB에서는 공칭 필터·이득·동적 범위와 기준 벡터를 확인하고, LTspice에서는 실제 schematic·S/H·ADC mapping과 회로 stress를 확인하며, XMODEL에서는 LTspice 정합·간섭·비이상성 및 부호 스트림 인계를 확인한다. 이후 디지털 모델과 RTL의 기능 일치를 점검하고, 기준을 만족하지 못하면 이 개발 단계에서 수정한다. 설계를 잠근 뒤에는 격리된 시험 데이터를 최초 1회만 평가하며 그 결과로 AFE 파라미터나 RTL 판정 구조를 다시 조정하지 않는다. 구현·보드 재생과 XMODEL–RTL end-to-end 검증은 잠금 설계의 추적성과 기능 등가성을 확인하는 단계다.
...
대표 NSR·CHF·ARR·AFF 60초 record에서는 ADC rail에 닿은 표본이 모두 0%였고, 가장 작은 rail 여유는 ARR의 1.019633440086 V였다 [CLM-015, CLM-024]. 그림 4c는 클래스별 최소 headroom, 그림 4d는 offset-binary code가 0·4095 rail에서 떨어져 분포하는 모습을 보여준다.
...
MATLAB 공칭 기준이 실제 schematic에서도 유지되는지는 XMODEL-aligned LTspice 회로로 별도 확인하였다. 그림 4a의 전체 응답은 약 46 dB 통과대역 이득, HPF roll-off, 60 Hz notch와 150 Hz LPF 위치가 정합함을 보여준다. 그림 4b는 60 Hz 주변의 narrow-band 응답을 확대한다. 1초 이후 MATLAB–LTspice ADC 비교는 MAE 0.678 LSB, RMS 2.225 LSB, correlation 0.998591이었다. 두 환경은 digital reference와 analog solver라는 구현 차이가 있으므로 bit-exact가 아니라 회로 설계 의도의 일치로 판정한다 [CLM-049].
...
디지털 분류기 앞단의 목적은 공개 ECG 전압을 증폭만 하는 것이 아니라, 기준선 이동과 전원선 간섭을 줄이고 1 kSPS signed 12-bit 스트림으로 일관되게 넘기는 것이다. 정본 LTspice 회로는 `FULL_AFE_ADC_SH_xmodel_aligned.asc`이며 기존 ±5 V 후보와 분리해 ±1.65 V 전원, XMODEL형 op-amp abstraction, `ECG+=patient100`, `ECG−=0 V`, 1 ms ADC aperture와 S/H를 적용하였다. 그림 7a는 전체 신호 경로와 비이상성 주입 위치, 그림 7b는 실제 graphical schematic이다 [CLM-034, CLM-049].
...
| LTspice 검증 항목 | 설계 목표 | 측정 결과 |
|---|---:|---:|
| HPF −3 dB | 0.4823 Hz | 0.481174 Hz |
| IA gain at 10 Hz | 201 V/V | 200.594 V/V |
| 60 Hz notch | power-line rejection | −83.557 dB |
| notch minimum | 60 Hz | 59.9995 Hz, −95.435 dB |
| LPF −3 dB | 150.15 Hz | 150.211 Hz |
| settled AFE 출력 | ±1.65 V 이내 | −0.0540~+0.2466 V, clipping 0 |
| ADC rail headroom | clipping 없음 | 1.403 V |
| S/H hold droop | 최소화 | 최대 0.0276 LSB |
...
**무엇을 검증했고 다음 블록에 어떻게 연결되는가.** MATLAB nodal dense sweep은 30–100 Hz 범위에서 60 Hz −83.5557 dB, local −3 dB bandwidth 11.721 Hz, 추정 Q 5.119를 보였다. 50 Hz 감쇠는 −1.131 dB이므로 50 Hz까지 제거한다고 해석할 수 없다 [CLM-029]. LTspice는 60 Hz −83.557 dB와 notch minimum 59.9995 Hz/−95.435 dB를, XMODEL 특성시험은 60 Hz notch 약 80 dB와 150 Hz 전체 대역폭을 확인했다. 그림 8의 전체 응답에서 60 Hz ideal zero는 시간영역 디지털 근사의 표시값이고, analog-style notch 수치는 그림 9의 active Twin-T dense sweep과 그림 4b의 MATLAB–LTspice 비교를 사용한다.
...
XMODEL의 첫 역할은 LTspice에서 확인한 회로 동작을 RTL과 함께 실행 가능한 SystemVerilog 행동모델로 재현하는 것이다. 동일한 `patient100` 10초 입력, ±1.65 V 범위, 1 kSPS signed 12-bit mapping을 적용해 LTspice S/H와 XMODEL ADC 10,000개 code를 비교하였다. 평균 오차 +0.0221 LSB, MAE 0.6445 LSB, RMS 1.3020 LSB, maximum absolute error 13 LSB, zero-lag correlation 0.999518, best lag 0, clipping 0이었다 [CLM-050].
...
서로 다른 analog solver의 출력은 모든 표본의 bit-exact 여부보다 code-error coverage로 판정한다. ±5 LSB를 nominal agreement band, ±10 LSB를 extended QRS-edge band로 정의했을 때 각각 9,874/10,000=98.74%, 9,989/10,000=99.89%가 범위 안에 있었다. 10,000개 중 11개만 ±10 LSB를 초과했으며 최대 오차는 13 LSB였다. 1 LSB는 약 0.806 mV이므로 5 LSB와 10 LSB는 ADC 입력 기준 약 4.03 mV와 8.06 mV다 [CLM-051].
...
기존 장시간 검증은 별도 질문을 다룬다. 36개 60초 구간에서 앞 3 s 정착부를 제외하고 emulator와 XMODEL을 비교한 평균 RMS 차이는 1.95 LSB, lag는 0이었다. 최대 편차는 QRS 급경사에서 30 LSB까지 나타났다 [CLM-014]. 이 결과는 LTspice↔XMODEL 10초 회로 정합과 입력 길이·비교 대상이 다르므로 두 수치를 합치거나 모두 bit-exact라고 표현하지 않는다.

XMODEL의 두 번째 역할은 공칭 MATLAB/LTspice에 없는 비이상성을 분리해 가하는 것이다. 전원선 간섭은 60 Hz에서 RMS 0.92 mV, 50 Hz에서 118 mV가 남았다. 이 설계의 target은 60 Hz이며 50 Hz에서는 center retuning과 별도 system 검증이 필요하다 [CLM-025]. R/C mismatch, GBW/VOS와 ADC 비이상성 결과는 표 7b와 같다.
...
MATLAB 공칭 검증은 선택한 입력이 ADC 상·하한 안에 있음을 보였고, LTspice는 이 사양을 실제 schematic으로 구현했을 때 목표 cutoff·gain·notch·S/H·ADC mapping과 clipping 0을 재현함을 보였다. 그림 15a와 15b는 동일한 10초 ECG에 대한 LTspice S/H와 XMODEL signed ADC 출력을 전체 구간과 2~3초 확대 구간에서 비교한다. 두 파형은 best lag 0에서 중첩되며 MAE 0.6445 LSB, RMS 1.3020 LSB, correlation 0.999518이었다 [CLM-049~CLM-051].
...
아날로그 입력에서 디지털 분류기까지의 경계를 먼저 고정하였다. ECG의 느린 기준선 변동은 10 MΩ·33 nF 고역통과필터로 줄이고, 100 kΩ·1 kΩ 저항을 사용한 3-op-amp IA에서 차동 신호를 201배 증폭한다. 60 Hz 전원선 간섭은 26.526 kΩ·100 nF 중심의 active Twin-T와 독립 buffer로 억제하고, 1 kΩ·1.06 µF 저역통과필터가 150.15 Hz보다 높은 성분을 제한한다. ±1.65 V, 12-bit, 1 kSPS ADC는 이 파형을 offset-binary 코드로 바꾼 뒤 signed two's-complement stream으로 변환하여 RTL에 전달한다. LTspice에서 HPF 0.481174 Hz, IA 200.594 V/V, 60 Hz −83.557 dB, LPF 150.211 Hz와 clipping 0을 확인했다. 수동 notch의 넓은 저지대역과 출력 loading, IA 수렴, LPF 차단주파수, CMRR margin과 ADC 기록 시점 문제는 설계 과정에서 실제로 수정한 항목이다 [CLM-028~CLM-030, CLM-049].

MATLAB은 이 공칭 회로의 이득·주파수응답·동적 범위·ADC 코드 분포를 확인하고 같은 입력을 반복할 수 있는 기준 벡터를 만든다. LTspice는 실제 schematic, 전원, S/H와 ADC mapping으로 회로 사양과 stress를 확인한다. XMODEL은 검증된 회로 계약을 SystemVerilog로 구현해 50/60 Hz 간섭, R/C mismatch, op-amp GBW와 VOS, ADC 비이상성 및 장시간 stream을 RTL 환경에서 가한다. LTspice–XMODEL 10초 10,000표본 비교는 MAE 0.6445 LSB, correlation 0.999518, ±5 LSB 98.74%, ±10 LSB 99.89%, clipping 0이었다. 세 환경의 역할을 분리했기 때문에 MATLAB의 공칭 설계 의도, LTspice 회로 구현, XMODEL의 행동모델·비이상성 검증, SHA256으로 고정된 signed stream과 RTL 상태 전이를 한 방향으로 추적할 수 있다. 이는 실제 PCB, fabricated ADC, post-layout 또는 실제 전극 측정을 대신하는 결과가 아니라 그 전 단계의 model-based mixed-signal handoff를 검증한 결과다 [CLM-001, CLM-002, CLM-015, CLM-031~CLM-034, CLM-049~CLM-052].
