...
![그림 2. 전체 연구·검증 workflow](../figures/final/FIG-02_recordwise_validation_workflow.svg)

*그림 2. 공개 ECG를 원본 기록 단위로 학습·검증·잠금 시험 데이터로 먼저 분리한다. 학습·검증 데이터만 MATLAB 사전검증과 병렬 XMODEL·디지털 RTL 설계, AFE–RTL 통합 검증, 가속기 성능 비교 및 FPGA 구현·보드 재생에 사용한다. 통합 검증에는 XMODEL과 RTL이 함께 입력되며, 성능 비교와 FPGA 검증은 디지털 RTL에서만 분기한다. 세 검증 결과가 기준을 충족하지 못하면 XMODEL·RTL 단계로 돌아가고, 충족하면 설계를 잠근다. 격리해 둔 시험 데이터는 그 뒤의 잠금 최종시험에서만 최초 1회 사용하며 결과를 설계에 되먹임하지 않는다. 아날로그 계층은 모델 기반이며 물리 측정 결과가 아니다. [근거: CLM-007, CLM-012, CLM-013, CLM-028; `source_of_truth/upstream_commits.yaml`; `components/digital_accelerator/configs/final_submission_locked_model.json`; `components/afe_xmodel/docs/integration_latest/afe_locked_rtl_integration_36case_compare.csv`]*

이 workflow에 적용되는 중요한 평가 원칙은 **설계·검증 반복과 최종 성능 평가의 분리**다. 원본 기록 단위 분할 직후 잠금 시험 데이터를 격리하고, MATLAB과 XMODEL·RTL 설계 및 세 검증 경로에는 학습·검증 데이터만 사용한다. MATLAB에서는 공칭 필터·이득·동적 범위와 기준 벡터를 확인하고, XMODEL에서는 간섭과 비이상성 및 부호 스트림 인계를 확인한다. 이후 정수 기준 구현과 RTL의 기능 일치, FPGA 통합의 `final_pred`·`final_mem` 일치를 차례로 점검한다. 이 과정에서 발견된 구현 오류는 XMODEL·RTL 단계로 돌아가 수정할 수 있다. 그러나 세 검증 경로를 통과해 설계를 잠근 뒤에는 격리된 시험 데이터를 최초 1회만 평가하며, 그 결과로 AFE 파라미터나 RTL 판정 구조를 다시 조정하지 않는다.
