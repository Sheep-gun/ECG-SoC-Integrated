...
def main():
 ...
 if a.reference_exact:
  for sub in ("include","src"):
   for p in (a.reference_exact/sub).glob("*"):
    if p.is_file():check(f"classifier_source:{sub}/{p.name}",sha(p)==sha(HERE/sub/p.name))
 else:
 ...
 if a.authority:
  ...
  check("fpga_cycle_summary_sha",sha(a.authority/"rtl_cycle_summary.json")=="a7b1dcc1c6e950aa89fc71175af78e6b82af831e13a88cba28af96ead9a95c18")
