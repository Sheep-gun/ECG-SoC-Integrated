...
#define SNN_PROFILE_LAST_DECISION_LO 0x148U
#define SNN_PROFILE_LAST_DECISION_HI 0x14cU
...
int main(void)
{
    ...
    print_profile_pair("profile_last_decision", SNN_PROFILE_LAST_DECISION_LO, SNN_PROFILE_LAST_DECISION_HI);

    /*
     * Machine-readable timing record.  UART output occurs after both counters
     * have stopped, so printing is outside the timed interval.
     *
     * core_cycles is the existing hardware decision-latency counter
     * (accelerator start acceptance through final decision).  system_cycles is
     * the existing total counter over the same board replay transaction.  The
     * current XSA has no independent AXI Timer, so the parser preserves these
     * scopes explicitly and does not relabel them as host latency.
     */
    xil_printf("BOARD_BENCH sample_count=%u core_cycles_lo=%u core_cycles_hi=%u "
               "system_cycles_lo=%u system_cycles_hi=%u final_pred=%u "
               "final_mem_NSR=%d final_mem_CHF=%d final_mem_ARR=%d final_mem_AFF=%d "
               "timing_source=accelerator_profile_counter timer_frequency_hz=100000000\r\n",
               samples_accepted,
               rd(SNN_BASE, SNN_PROFILE_LAST_DECISION_LO),
               rd(SNN_BASE, SNN_PROFILE_LAST_DECISION_HI),
               rd(SNN_BASE, SNN_PROFILE_TOTAL_LO),
               rd(SNN_BASE, SNN_PROFILE_TOTAL_HI),
               final_pred,
               rd_s32(SNN_BASE, SNN_FINAL_MEM_NSR),
               rd_s32(SNN_BASE, SNN_FINAL_MEM_CHF),
               rd_s32(SNN_BASE, SNN_FINAL_MEM_ARR),
