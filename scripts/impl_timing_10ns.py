...
def write_inputs(profile_en: int) -> Path:
    rd = run_dir(profile_en)
    rd.mkdir(parents=True, exist_ok=True)

    clock_xdc = rd / "clock_10ns.xdc"
    clock_xdc.write_text(
...
def parse_route_status(path: Path) -> dict[str, bool | None]:
    ...
    has_unrouted = bool(re.search(r"unrouted", text, re.IGNORECASE)) and not fully_routed
...
def write_report(summary: dict[str, Any]) -> None:
    ...
    (OUT / "impl_timing_10ns_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    ...
    for profile_key in sorted(summary["runs"], key=int):
        ...
        lines.append(
            ...
            f"{timing.get('pulse_width_worst_slack_ns')} |"
        )
    ...
    (OUT / "impl_timing_10ns_report.md").write_text("\n".join(lines), encoding="utf-8")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run OOC place/route timing for snn_ecg_30min_final_top at 10 ns.")
    parser.add_argument(
        "--profile-en",
        choices=["0", "1", "all"],
        default="1",
...
def main() -> None:
    ...
    print(OUT / "impl_timing_10ns_summary.json")
