...
- 0.099/0.183/0.271 W 또는 0.006590360700/0.003061096500 J를 physical board 측정값으로 표현하는 문장
...
Accelerator-benefit package는 digital commit `6298a8e...`에서 반입하였다. FPGA active-core latency는 실보드의 measured `profile_total`과 `profile_input_wait` 차에서 derived한 36.012900 ms이며 Exact C++ 1,777.699800 ms 대비 speedup은 49.362861641×다. UART-paced raw interval 187,144.750920 ms는 transport diagnostic으로만 보존한다. Integrated-system compute latency/speedup/energy는 preload와 독립 timer가 없어 미측정이다. direct-100 MHz Pure RTL의 Total/Dynamic/Static 0.183/0.085/0.097 W와 integrated system 0.271 W는 Vivado vectorless estimate이다. 100 MHz Total energy 0.006590360700 J와 active dynamic energy 0.003061096500 J만 clock-matched DERIVED_ESTIMATE이며, 1 MHz Pure RTL 0.099 W는 power-only로 보존한다. Physical board input power와 measured energy는 외부 전력계가 없어 미측정이다.
