...
30분 구간은 원천 record의 label을 기계적으로 상속하지 않았다. 원천 DB label과 가용한 beat/rhythm annotation을 대조해 각 구간이 해당 클래스의 박동 및 리듬 증거를 충분히 포함하는지 확인하고 라벨 대표성을 점검했다.

초기 분석에서는 같은 annotation으로 박동 위치, RR 간격, 조기·지연 박동, 리듬 불규칙성, 진폭, QRS 폭과 파형 굴곡 후보를 계산했다. 클래스별 분포와 분리 가능성을 비교해 PNN, RDM, Ectopic Evidence, DSCR, RAM, QRS MAF, RBBB-like에 대응하는 증거를 선정했다.

annotation은 30분 구간의 라벨 대표성 및 데이터 품질 점검과 feature 후보 선정에만 사용했다. 최종 RTL은 annotation 파일을 입력으로 받지 않으며 signed 12-bit ECG stream만으로 사건과 증거를 생성한다.
