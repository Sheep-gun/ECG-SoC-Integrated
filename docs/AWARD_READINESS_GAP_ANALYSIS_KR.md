...
| 2 | Record-wise/LORO | record diversity 약점 노출 및 보강 | `recordwise_eval_summary.md`, `loro_eval_summary.md` |
...
- 현재 정확도는 chunk-level balanced dataset 기준이며, record-wise audit 결과를 별도 제시한다.

## 5. 최종 보강 체크리스트

- [x] record-wise split audit
- [x] fixed-model record-wise regrouping result
- [x] fixed-model leave-one-record-out report
