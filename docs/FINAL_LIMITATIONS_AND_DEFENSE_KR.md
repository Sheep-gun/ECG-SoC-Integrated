...
| full dataset board replay | 아직 아님. locked model 기준 bitstream/XSA/ELF build 완료, actual UART replay pending |
...
| Full-record replay | locked bitstream/XSA/ELF rebuilt, actual locked UART transcript pending |

기존 `reports/board_replay/transcripts/test_case0_nsr_uart_full_replay.txt`는 legacy board transport evidence이다. Locked model 기준 transcript는 아직 없다.

### Q9. full-record board replay와 smoke test 차이는 무엇인가?

Smoke는 IP 연결, register map, feeder, interrupt, final readback을 빠르게 확인하기 위한 16-sample deterministic test이다. Full-record replay는 실제 30분 ECG `.mem` 1,800,000 samples를 board에 보내고, 30 snapshots와 final decision이 locked Python/XSim expected와 일치하는지 확인하는 단계이다. Locked model 기준 이 단계는 아직 transcript 확보가 필요하다.
