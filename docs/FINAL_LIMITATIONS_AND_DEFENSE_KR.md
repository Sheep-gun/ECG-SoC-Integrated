...
| Full-record replay | class-wise 30분 UART replay 완료, final_pred/final_mem 4/4 exact match |
...
Smoke는 IP 연결, register map, feeder, interrupt, final readback을 빠르게 확인하기 위한 16-sample deterministic test이다. Full-record replay는 실제 30분 ECG `.mem` 1,800,000 samples를 board에 보내고, 30 snapshots와 final decision이 locked full-top XSim expected와 일치하는지 확인하는 단계이다. Locked model 기준 class-wise replay는 수행됐고 NSR/CHF/ARR/AFF 각 1건에서 final_pred와 final_mem이 4/4 exact match로 확인됐다.
...
- strict record-wise final_test accuracy
