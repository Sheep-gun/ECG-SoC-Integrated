...
Resource와 timing closure에 더해 Nexys A7-100T 36-case hardware counter를 반영한다. Active-core는 `profile_total-profile_input_wait=3,601,290 cycles`, 36.012900 ms이고 처리량은 49,982,089.751172 samples/s, Exact C++ 대비 speedup은 49.362861641×다. UART-paced raw interval 187,144.750920 ms는 transport diagnostic으로만 보존하며 integrated-system compute timing은 미측정이다. 성능 조건과 일치하는 direct-100 MHz timing-closed route의 네 class 실제 ECG burst SAIF 중앙값은 accelerator+static/hierarchy dynamic/device static 0.149500/0.052500/0.097000 W이고 WNS +0.035 ns로 MET이다. Routed-net SAIF match는 약 12%이며 미매칭 net은 vectorless다. `0.149500 W × 0.0360129 s`로 구한 5.383929 mJ의 활성 에너지를 30분 주기 1,800초로 환산하면, 유휴 시간에 가속기 정적전력까지 완전히 제거하는 power-gating 가정에서 연산 코어 평균전력은 2.991071 µW이다. 이 값은 현재 FPGA 전력이나 전체 wearable 전력이 아니라 저전력 ASIC 구현 가능성을 나타내는 이상적 파생값이며, 메모리·상태 유지·전원 제어·기동·누설과 AFE·ADC·MCU·BLE·PMIC 전력을 포함하지 않는다. 기존 1 MHz 0.099 W와 MicroBlaze system 0.271 W는 각각 별도 vectorless power-only operating point이며 physical board input power는 미측정이다.
