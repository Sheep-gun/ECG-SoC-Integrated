...
Resource와 timing closure에 더해 Nexys A7-100T 36-case hardware counter를 반영한다. Active-core는 `profile_total-profile_input_wait=3,601,290 cycles`, 36.012900 ms이고 처리량은 49,982,089.751172 samples/s, Exact C++ 대비 speedup은 49.362861641×다. UART-paced raw interval 187,144.750920 ms는 transport diagnostic으로만 보존하며 integrated-system compute timing은 미측정이다. 성능 조건과 일치하는 direct-100 MHz Pure RTL의 Vivado post-implementation vectorless estimate는 Total/Dynamic/Static 0.183/0.085/0.097 W이고 timing은 WNS +0.035 ns로 MET이다. 기존 1 MHz 0.099 W와 MicroBlaze system 0.271 W는 각각 별도 power-only operating point이며 physical board input power는 미측정이다.
