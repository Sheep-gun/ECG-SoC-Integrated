...
| `fig_dynamic_range_headroom` | 대표 NSR/CHF/ARR/AF 입력에 대한 ADC rail 대비 AFE 출력 headroom을 나타낸다. 모든 클래스에서 clipping ratio가 0%임을 nominal 기준에서 확인하기 위한 figure이다. 본 figure는 SystemVerilog XMODEL 구현 전 MATLAB nominal reference 결과이며, transistor-level, PCB-level, silicon-level 검증 또는 MATLAB-vs-XMODEL 등가성 검증을 의미하지 않는다. |
