...
MATLAB AFE+ADC nominal pre-validation과 AFE+ADC SystemVerilog XMODEL stress/integration verification은 teammate repository에서 관리한다. 이 repo는 upstream AFE/XMODEL flow가 제공하는 signed 12-bit ECG stream contract부터 시작하여 digital accelerator path를 검증한다. 따라서 본 결과는 실제 전극 기반 ECG 측정, physical AFE PCB 측정, ADC silicon 측정, CMOS layout/post-layout 검증, clinical diagnosis validation을 의미하지 않는다.
...
본 보고서의 초점은 analog chain 자체가 아니라 **signed 12-bit ECG stream 이후의 digital accelerator IP**이다. 목표는 높은 계산량의 generic neural network가 아니라, 1 kSPS ECG stream을 직접 받아 60초 snapshot과 30분 final decision을 처리하는 low-resource biomedical streaming accelerator IP를 구현하고 검증하는 것이다.
...
전체 merged-paper flow는 MATLAB nominal pre-validation, AFE+ADC XMODEL verification, signed 12-bit ECG stream, SNN Snapshot Readout, Final Membrane Readout, RTL/IP/FPGA validation으로 이어진다. 이 중 본 repo는 signed 12-bit stream 이후의 digital accelerator path를 소유하며, 아래 cross-repo ownership table로 책임 경계를 고정한다.
...
## 4. Digital Input Contract and Upstream AFE Dependency

본 repo의 digital input contract는 **signed 12-bit ECG stream**이다. Upstream AFE/XMODEL repo가 이 stream을 제공하고, 본 repo는 그 이후의 digital accelerator behavior를 검증한다.

Upstream analog chain은 merged paper에서 한 줄로만 연결한다: `HPF 0.482 Hz -> IA x201 -> 60 Hz notch -> LPF 150 Hz -> 12-bit ADC`.
...
| Upstream AFE-to-locked RTL integration evidence | AFE+ADC XMODEL output stream을 locked RTL에 태워 digital expected와 비교 | XMODEL teammate repo, using this repo's canonical cadence |

따라서 본 repo의 main body는 MATLAB filter validation, XMODEL stress, PLI, mismatch, op-amp robustness를 상세 검증하지 않는다. 해당 내용은 teammate repo의 evidence로 연결하고, 본 repo는 signed 12-bit stream 이후의 locked protocol, RTL/XSim, Vivado/IP-XACT, Vitis/MicroBlaze board replay를 검증한다.
...
이 repo의 scope는 digital accelerator IP이다. MATLAB nominal pre-validation은 upstream analog-chain intent를 정리하고, XMODEL repo는 AFE+ADC stress 및 AFE-to-locked RTL integration evidence를 유지한다. 본 repo는 그 결과가 합의한 signed 12-bit ECG stream contract와 `sample_gap_cycles=2` canonical full-top XSim cadence를 기준으로 locked RTL/IP/FPGA 검증을 수행한다.
...
- `vin` reconstruction과 AFE+ADC XMODEL stream provenance는 upstream teammate repos에서 관리되며, 본 repo는 signed 12-bit stream contract 이후를 검증한다.
...
- MATLAB nominal filter validation과 AFE+ADC XMODEL stress verification은 본 repo의 소유 범위가 아니다.
...
본 repo는 signed 12-bit ECG-stream SNN accelerator IP의 **digital hardware validation repository**로 정리되었다. AFE+ADC XMODEL과 analog/mixed-signal verification은 upstream teammate repos에서 유지하고, 본 repo는 locked strict record-wise protocol, RTL/XSim, Vivado/IP-XACT, Vitis/MicroBlaze board replay evidence를 제공한다.

최종 모델은 train/validation으로 lock하고 final_test를 1회만 평가했으며, Python/XSim/Vivado/IP-XACT/Vitis evidence가 동일 locked model 기준으로 연결되어 있다. 대회 제출과 merged paper 관점에서는 upstream MATLAB/XMODEL evidence가 signed 12-bit stream provenance를 제공하고, 본 repo가 그 stream 이후의 accelerator IP와 board-level digital replay evidence를 담당한다.
