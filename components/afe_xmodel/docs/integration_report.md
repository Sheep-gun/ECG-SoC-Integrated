...
- **ARR만 AFE 경로에서 ARR→AF 플립**. 전수 격리 실험(scripts/filter_mem.py+run_digref_filt.sh):
  AFE 등가 디지털 필터가 AFE 통합 결과를 **정확히 재현**(full→NSR2 CHF1 ARR3 AF3) → AFE 효과
  ...
  (팀원 RTL ARR 점수여유 score_arr 62704 vs score_af 33899를 뒤집음. 입력이 이미 디지털화된
