...
본 프로젝트는 공개 digitized ECG record를 analog-equivalent `vin`으로 재구성하고, AFE+ADC XMODEL을 통과시켜 signed 12-bit stream을 생성한 뒤, 이를 SNN-inspired ECG Classification Accelerator IP Core에 입력하여 NSR, CHF, ARR, AF 4-class 장시간 ECG classification을 수행하는 FPGA/VLSI engineering prototype이다.
...
이 stream은 RTL/IP에 입력되어 60초 단위 Snapshot Readout을 만든다. 이후 30개 snapshot evidence를 Final Membrane Readout이 누적하고, class별 final membrane을 WTA로 비교하여 NSR/CHF/ARR/AF 중 하나를 출력한다.
