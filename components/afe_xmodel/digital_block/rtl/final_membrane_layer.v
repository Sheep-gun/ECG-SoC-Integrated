...
//   -> structural guard/rescue/silent-AF overlay
//   -> final WTA with strict ">" tie-break order NSR, CHF, ARR, AF.
//
...
module final_membrane_layer(
    ...
    input signed [63:0] class_mem_af,
    ...
    output reg signed [31:0] final_mem_af
);
    ...
    (* keep = "true", dont_touch = "true" *) reg [5:0] dec_count_af;
    ...
    wire [5:0] count_af_next = pred_count_af + ((pred_valid && (pred_class == CLS_AF)) ? 6'd1 : 6'd0);
    ...
    function [1:0] argmax_count4;
        ...
        begin
            ...
            if (d > best) begin best = d; argmax_count4 = CLS_AF; end
        end
    endfunction
    ...
    function [1:0] argmax_score4;
        ...
        begin
            ...
            if (d > best) begin best = d; argmax_score4 = CLS_AF; end
        end
    endfunction

    wire [1:0] base_pred_comb = argmax_count4(dec_count_nsr, dec_count_chf, dec_count_arr, dec_count_af);
    wire base_is_nsr = (base_pred_comb == CLS_NSR);
    wire base_is_chf = (base_pred_comb == CLS_CHF);
    wire base_is_arr = (base_pred_comb == CLS_ARR);
    wire base_is_af = (base_pred_comb == CLS_AF);
    ...
    wire signed [31:0] dec_count_af_s = $signed({26'd0, dec_count_af});
    wire signed [31:0] dec_nsr_minus_arr_s = dec_count_nsr_s - dec_count_arr_s;
    wire signed [31:0] dec_chf_minus_af_s = dec_count_chf_s - dec_count_af_s;
    ...
    wire rdm_code_ge_base_af_low = (dec_rdm_valid != 32'd0) && (rdm_code_ext >= rdm_threshold_11);
    ...
    wire strong_chf =
        ...
        ((dec_count_chf >= STRICT_RW_BASE_STRONG_CHF_COUNT_GE) ||
         (dec_chf_minus_af_s >= STRICT_RW_BASE_STRONG_CHF_MARGIN_GE));

    wire gate_base_af_low =
        (STRICT_RW_BASE_AF_LOW_ENABLE != 0) &&
        base_is_chf &&
        (dec_abnormal <= STRICT_RW_BASE_AF_LOW_ABN_LE) &&
        (dec_morphology <= STRICT_RW_BASE_AF_LOW_MORPH_LE) &&
        (dec_rbbb_like <= STRICT_RW_BASE_AF_LOW_RBBB_LE) &&
        rdm_code_ge_base_af_low &&
        (dec_count_chf >= STRICT_RW_BASE_AF_LOW_CHF_COUNT_GE);
    ...
    wire gate_base_chf_from_af =
        base_is_af &&
        (dec_count_chf >= STRICT_RW_BASE_CHF_FROM_AF_CHF_COUNT_GE) &&
        (dec_morphology <= STRICT_RW_BASE_CHF_FROM_AF_MORPH_LE) &&
        (dec_qrs_maf <= STRICT_RW_BASE_CHF_FROM_AF_QRS_LE) &&
        (dec_rbbb_like <= STRICT_RW_BASE_CHF_FROM_AF_RBBB_LE);

    wire gate_base_af =
        (dec_count_arr < STRICT_RW_BASE_AF_BLOCK_ARR_COUNT_GE) &&
        (base_is_chf || base_is_arr) &&
        (!strong_chf) &&
        (dec_count_af >= STRICT_RW_BASE_AF_COUNT_GE) &&
        (dec_chf_minus_af_s <= STRICT_RW_BASE_AF_CHF_MARGIN_LE) &&
        (dec_rhythm >= STRICT_RW_BASE_AF_RHYTHM_GE) &&
        (dec_ectopic_pair >= STRICT_RW_BASE_AF_ECP_GE) &&
        (dec_ectopic_pair <= STRICT_RW_BASE_AF_ECP_LE);

    wire gate_base_arr =
        (!base_is_arr) &&
        (!strong_nsr) &&
        (dec_count_arr >= STRICT_RW_BASE_ARR_COUNT_GE) &&
        (dec_count_af <= STRICT_RW_BASE_ARR_AF_COUNT_LE) &&
    ...
    wire gate_base_af_rescue_source_ok =
        ((STRICT_RW_BASE_AF_RESCUE_SOURCE_CHF != 0) && base_is_chf) ||
        ((STRICT_RW_BASE_AF_RESCUE_SOURCE_ARR != 0) && base_is_arr);

    wire gate_base_af_rescue =
        (STRICT_RW_BASE_AF_RESCUE_ENABLE != 0) &&
        gate_base_af_rescue_source_ok &&
        (dec_count_arr < STRICT_RW_BASE_AF_RESCUE_ARR_COUNT_LT) &&
        (dec_count_af >= STRICT_RW_BASE_AF_RESCUE_COUNT_GE) &&
        (dec_chf_minus_af_s <= STRICT_RW_BASE_AF_RESCUE_CHF_MARGIN_LE) &&
        (dec_rhythm >= STRICT_RW_BASE_AF_RESCUE_RHYTHM_GE) &&
        (dec_ectopic_pair >= STRICT_RW_BASE_AF_RESCUE_ECT_GE);

    wire gate_base_arr_low =
        (STRICT_RW_BASE_ARR_LOW_ENABLE != 0) &&
        (!base_is_arr) &&
        (dec_count_arr >= STRICT_RW_BASE_ARR_LOW_COUNT_GE) &&
        (dec_count_af <= STRICT_RW_BASE_ARR_LOW_AF_COUNT_LE) &&
    ...
    wire signed [31:0] base_mem_chf_comb =
        base_seed_chf -
        (gate_base_af_low_r ? STRICT_RW_BASE_AF_LOW_INHIBIT_CHF : 32'sd0) -
        (gate_base_nsr_from_chf_r ? STRICT_RW_BASE_NSR_FROM_CHF_INHIBIT_CHF : 32'sd0) +
        (gate_base_chf_from_af_r ? STRICT_RW_BASE_CHF_FROM_AF_BOOST : 32'sd0) -
        (gate_base_af_r ? STRICT_RW_BASE_AF_INHIBIT_CHF : 32'sd0) -
        (gate_base_arr_r ? STRICT_RW_BASE_ARR_INHIBIT_CHF : 32'sd0) -
        (gate_base_af_rescue_r ? STRICT_RW_BASE_AF_RESCUE_INHIBIT_CHF : 32'sd0) -
        (gate_base_arr_low_r ? STRICT_RW_BASE_ARR_LOW_INHIBIT_CHF : 32'sd0);

    wire signed [31:0] base_mem_arr_comb =
        base_seed_arr -
        (gate_base_af_r ? STRICT_RW_BASE_AF_INHIBIT_ARR : 32'sd0) +
        (gate_base_arr_r ? STRICT_RW_BASE_ARR_BOOST : 32'sd0) -
        (gate_base_af_rescue_r ? STRICT_RW_BASE_AF_RESCUE_INHIBIT_ARR : 32'sd0) +
        (gate_base_arr_low_r ? STRICT_RW_BASE_ARR_LOW_BOOST : 32'sd0);

    wire signed [31:0] base_mem_af_comb =
        base_seed_af +
        (gate_base_af_low_r ? STRICT_RW_BASE_AF_LOW_BOOST : 32'sd0) -
        (gate_base_chf_from_af_r ? STRICT_RW_BASE_CHF_FROM_AF_INHIBIT_AF : 32'sd0) +
        (gate_base_af_r ? STRICT_RW_BASE_AF_BOOST : 32'sd0) -
        (gate_base_arr_r ? STRICT_RW_BASE_ARR_INHIBIT_AF : 32'sd0) +
        (gate_base_af_rescue_r ? STRICT_RW_BASE_AF_RESCUE_BOOST : 32'sd0) -
        (gate_base_arr_low_r ? STRICT_RW_BASE_ARR_LOW_INHIBIT_AF : 32'sd0);

    wire gate_struct_af_persistence =
        (dec_count_af >= STRICT_RW_FINAL_AF_CNT) &&
        (dec_count_chf >= STRICT_RW_FINAL_AF_CHF_CNT) &&
        (dec_morphology <= STRICT_RW_FINAL_AF_MORPH_LE) &&
        (dec_rhythm >= STRICT_RW_FINAL_AF_RHYTHM_GE) &&
        (dec_qrs_maf <= STRICT_RW_FINAL_AF_QRS_LE);
    ...
    wire gate_struct_arr_over_af =
        (dec_count_af >= STRICT_RW_FINAL_ARR_AF_AF_CNT) &&
        (dec_count_arr >= STRICT_RW_FINAL_ARR_AF_ARR_CNT) &&
        (dec_morphology >= STRICT_RW_FINAL_ARR_AF_MORPH_GE) &&
        (dec_ectopic_pair >= STRICT_RW_FINAL_ARR_AF_ECT_GE) &&
        (dec_qrs_maf <= STRICT_RW_FINAL_ARR_AF_QRS_LE);

    wire gate_struct_silent_af =
        (dec_count_chf >= STRICT_RW_SILENT_AF_CHF_CNT_GE) &&
        (dec_count_nsr <= STRICT_RW_SILENT_AF_NSR_CNT_LE) &&
        (dec_count_arr == STRICT_RW_SILENT_AF_ARR_CNT_EQ) &&
        (dec_count_af == STRICT_RW_SILENT_AF_AF_CNT_EQ) &&
        (dec_beat <= STRICT_RW_SILENT_AF_BEAT_LE) &&
        (dec_morphology <= STRICT_RW_SILENT_AF_MORPH_LE) &&
        (dec_rhythm <= STRICT_RW_SILENT_AF_RHYTHM_LE) &&
        (dec_abnormal <= STRICT_RW_SILENT_AF_ABN_LE) &&
        (dec_qrs_maf <= STRICT_RW_SILENT_AF_QRS_LE) &&
        (dec_ectopic_pair == STRICT_RW_SILENT_AF_ECT_EQ) &&
        (dec_ram_code == STRICT_RW_SILENT_AF_RAM_EQ);
    ...
    wire signed [31:0] struct_mem_chf_comb =
        base_mem_chf -
        (gate_struct_af_persistence ? STRICT_RW_FINAL_AF_INH_CHF : 32'sd0) -
        ...
        (gate_struct_silent_af ? STRICT_RW_SILENT_AF_INH_CHF : 32'sd0);

    wire signed [31:0] struct_mem_arr_comb =
        base_mem_arr -
        (gate_struct_af_persistence ? STRICT_RW_FINAL_AF_INH_ARR : 32'sd0) -
        ...
        (gate_struct_arr_over_af ? STRICT_RW_FINAL_ARR_AF_BOOST : 32'sd0);

    wire signed [31:0] struct_mem_af_comb =
        base_mem_af +
        (gate_struct_af_persistence ? STRICT_RW_FINAL_AF_BOOST : 32'sd0) -
        (gate_struct_arr_over_nsr ? STRICT_RW_FINAL_ARR_NSR_INH_AF : 32'sd0) -
        (gate_struct_arr_over_af ? STRICT_RW_FINAL_ARR_AF_INH_AF : 32'sd0) +
        (gate_struct_silent_af ? STRICT_RW_SILENT_AF_BOOST : 32'sd0);
        ...
        if (rst || clear) begin
            ...
            case (final_pipe_stage)
                ...
                FM_BASE: begin
                    ...
                    base_seed_af <= dec_count_af_s;
                    gate_base_af_low_r <= gate_base_af_low;
                    gate_base_nsr_from_chf_r <= gate_base_nsr_from_chf;
                    gate_base_chf_from_af_r <= gate_base_chf_from_af;
                    gate_base_af_r <= gate_base_af;
                    gate_base_arr_r <= gate_base_arr;
                    gate_base_af_rescue_r <= gate_base_af_rescue;
                ...
                FM_STRUCT: begin
                    ...
                    struct_mem_af <= struct_mem_af_comb;
                ...
                FM_WTA: begin
                    if (final_pipe_chunk) begin
                        ...
                        final_mem_af <= struct_mem_af;
                        final_pred_class <= argmax_score4(struct_mem_nsr, struct_mem_chf, struct_mem_arr, struct_mem_af);
                        final_valid <= 1'b1;
                    end
                    final_pipe_chunk <= 1'b0;
                    final_pipe_stage <= FM_IDLE;
