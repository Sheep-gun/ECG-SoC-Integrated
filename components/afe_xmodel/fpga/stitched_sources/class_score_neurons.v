...
module class_score_neurons #(
    ...
    parameter ENABLE_RBBB_QRS_DELAY_GATE = 0,

    parameter W_RBBB_DELAY_NSR_INH = 150000,

    parameter W_RBBB_DELAY_ARR_BOOST = 0,
    ...
    always @* begin
        ...
        score_nsr_before_suppress = score_nsr_next;

        nsr_top_before_suppress = (score_nsr_next >= score_chf_next) &&
        ...
        nsr_chf_block_before_suppress = (score_chf_next >= (score_nsr_next - T_NSR_GATE_CHF_BLOCK_MARGIN_S)) ||

                                        (score_chf_next >= (score_arr_next + T_NSR_GATE_CHF_OVER_ARR_MARGIN_S));

        nsr_suppress_applied = (ENABLE_NSR_NORMALITY_GATE != 0) &&

                               segment_done &&

                               nsr_suppress_spike &&

                               nsr_top_before_suppress &&

                               nsr_near_arr_before_suppress &&

                               !nsr_chf_block_before_suppress;

        if (nsr_suppress_applied)

            score_nsr_next = score_nsr_next - W_NSR_SUPPRESS_S;
        ...
        rbbb_late_chf_block_before = (score_chf_next >= (score_nsr_next - T_RBBB_LATE_CHF_BLOCK_MARGIN_S)) ||
                                      ...
                                      (score_chf_next >= score_arr_next);

        rbbb_lateslope_applied = (ENABLE_RBBB_LATESLOPE_GATE != 0) &&

                                  segment_done &&

                                  rbbb_lateslope_gate_spike &&

                                  rbbb_late_top_nsr_before &&

                                  !rbbb_late_chf_block_before;

        if (rbbb_lateslope_applied) begin
        ...
        rbbb_delay_chf_block_before = (score_chf_next >= (score_nsr_next - T_RBBB_DELAY_CHF_BLOCK_MARGIN_S)) ||
                                      ...
                                      ((RBBB_DELAY_CHF_OVER_ARR_BLOCK != 0) && (score_chf_next >= score_arr_next));

        rbbb_qrs_delay_applied = (ENABLE_RBBB_QRS_DELAY_GATE != 0) &&

                                 segment_done &&

                                 rbbb_qrs_delay_segment_spike &&

                                 rbbb_delay_top_nsr_before &&

                                 !rbbb_delay_chf_block_before;

        if (rbbb_qrs_delay_applied) begin
