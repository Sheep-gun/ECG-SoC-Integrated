...
EXPECTED_HASHES={
 "models/digital_equivalence/results/benchmark_dataset_manifest.csv":"4965b8a098617d6138e4e56e2b45febda20706b031e9bbaa2558d874517dee72",
...
def text_sha(p:Path)->str:return hashlib.sha256(p.read_bytes().replace(b"\r\n",b"\n")).hexdigest()
def rows(p:Path)->list[dict[str,str]]:
    with p.open(newline="",encoding="utf-8-sig") as f:return list(csv.DictReader(f))
def git_ok(*args:str)->bool:
    return bool(GIT) and subprocess.run([GIT,*args],cwd=REPO,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode==0
def main()->int:
    ...
    for rel,want in EXPECTED_HASHES.items():
        actual=text_sha(REPO/rel)
        check("artifact_sha256:"+rel,actual==want,actual)
    ...
    manifest=rows(REPO/"models/digital_equivalence/results/benchmark_dataset_manifest.csv")
    ...
    header=EXACT/"include/locked_parameters.hpp";before=text_sha(header)
    ...
    after=text_sha(header);check("parameter_regeneration",gen.returncode==0 and before==after,gen.stdout.strip())
